...
//   static gtsam::SO<2>::TangentVector Logmap(const SO2& so2, gtsam::OptionalJacobian<1, 1> H = boost::none)
//   {
//     const Eigen::Matrix<double, 2, 2> mat{ so2.matrix() };
//     const double r21{ mat(1, 0) };
//     const double r11{ mat(0, 0) };
//     gtsam::Rot2::Logmap(const Rot2& r, ChartJacobian H = boost::none);
...
BOOST_AUTO_TEST_CASE(optimizer_find_x0_test)
{
  ...
  gtsam::noiseModel::Base::shared_ptr noise{ gtsam::noiseModel::Isotropic::Sigma(1, 1) };

  graph.addPrior(key_x1, x1, noise);
  graph.addPrior(key_xdot, xdot0, noise);

  // for (auto key : graph.keys())
  //   std::cout << gtsam::DefaultKeyFormatter(key) << " ";
  // std::cout << "\n";
  // prx::fg::indeterminant_linear_system_helper(graph, initial_values);
  gtsam::Values values = gtsam::GaussNewtonOptimizer(graph, initial_values).optimize();
  // values.print();
