...
BOOST_AUTO_TEST_CASE(euler_dorpi_test)
{
  ...
  {
    const Eigen::VectorXd pt_euler{ euler_traj.at(i)->as<Eigen::VectorXd>() };
    const Eigen::VectorXd pt_dorpi{ traj_dorpi.at(i)->as<Eigen::VectorXd>() };

    BOOST_REQUIRE_SMALL((pt_euler - pt_dorpi).norm(), 0.1);
    BOOST_REQUIRE(pt_dorpi.norm() > 0.0);
    BOOST_REQUIRE(pt_euler.norm() > 0.0);
  }
}