...
BOOST_AUTO_TEST_CASE(euler_dorpi_test)
{
  ...
  prx::simulation_step = 0.0000001;
  sg->propagate(curr_state, plan_euler, euler_traj);

  prx::simulation_step = 0.0001;
  ...
  for (double i = 0; i <= 1; i += 0.01)
  {
    ...
    BOOST_CHECK(dist(pt_euler, pt_dorpi) < 0.01);
    std::cout << "euler: " << pt_euler << std::endl;
    std::cout << "dorpi: " << pt_dorpi << std::endl;
  }
}