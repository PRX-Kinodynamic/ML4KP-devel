...
std::function<double(double)> Kx_function = [](double x)
{
	return 1.0 - std::exp(-2.0 * std::pow(x, 2));
};

std::tuple<bool, double, double, std::string> kolmogorov_smirnov_test(
					std::function<double()>& random_function, std::function<double(double)>& Fx) 
{
  ...
  double cutoff_low = std::sqrt(0.5 * std::log(1.0 / (1.0 - p_low))) -
                      1.0 / (6.0 * sqrtNumReps);
  double cutoff_high = std::sqrt(0.5 * std::log(1.0 / (1.0 - p_high))) -
                       1.0 / (6.0 * sqrtNumReps);
  ...
  return std::make_tuple(
  			cutoff_low <= K_plus && K_plus <= cutoff_high && cutoff_low <= K_minus && K_minus <= cutoff_high,
...
BOOST_AUTO_TEST_CASE(baseRandomTestUsingStdUniformDistribution) 
{
    ...
    BOOST_CHECK_MESSAGE(k_minus_kolmogorov_smirnov_test_output, "K^- Kolmogorov-Smirnov test failed!" << failed_msg_k_minus);
    BOOST_CHECK_MESSAGE(k_plus_kolmogorov_smirnov_test_output, "K^+ Kolmogorov-Smirnov test failed!" << failed_msg_k_plus);
}
...
{
    prx::init_random(std::random_device()());
    std::function<double()> uniform_random_wrapper_0_100 = []()
    {
    	return prx::uniform_random(0,100);
    };
    std::function<double()> uniform_random_wrapper_m100_100 = []()
    {
    	return prx::uniform_random(-100,100);
    };
    std::function<double()> uniform_random_wrapper_m100_0 = []()
    {
    	return prx::uniform_random(-100,0);
    };
	...
	std::function<double(double)> uniform_function_m100_0 = 
    								std::bind(uniform_function, std::placeholders::_1, -100, 0);
	...
	bool kolmogorov_smirnov_test_output_0_100, 
			k_minus_kolmogorov_smirnov_test_output_0_100, k_plus_kolmogorov_smirnov_test_output_0_100;
	bool kolmogorov_smirnov_test_output_m100_100, 
			k_minus_kolmogorov_smirnov_test_output_m100_100, k_plus_kolmogorov_smirnov_test_output_m100_100;
	bool kolmogorov_smirnov_test_output_m100_0, 
			k_minus_kolmogorov_smirnov_test_output_m100_0, k_plus_kolmogorov_smirnov_test_output_m100_0;
