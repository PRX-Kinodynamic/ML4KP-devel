...
void compare_outputs(std::vector<std::vector<double>> v1, std::vector<std::vector<double>> v2, const int n)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      std::cout << "Conflict! v1 = " << v1.at(i).at(0) << " " << v1.at(i).at(1) << " v2 = " << v2.at(i).at(0) << " "
                << v2.at(i).at(1) << std::endl;
    }
  }
}
...
// #define SECOND_ORDER
    ...
    for (int i = 0; i < num_trials; i++)
    {
      ...
      for (int idx = 0; idx < num_predictions; idx++)
      {
        std::vector<double> out = { controller_output[idx][0].item<double>(),
                                    controller_output[idx][1].item<double>() };
