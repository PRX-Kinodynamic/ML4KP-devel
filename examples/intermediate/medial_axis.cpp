...
const double map_resolution = 0.25;
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    auto obstacles = load_obstacles("environments/simple_obstacle.yaml");
    ...
    init_random(231192);
    std::string plant_name = "2D_Point";
    ...
    std::vector<double> goal_vec = { (20.0) / map_resolution, (20.0) / map_resolution };
    ...
    //     vis_group->add_vis_infos(info_geometry_t::FULL_LINE, traj, plant_name + "/body",
    //                              context.first->get_state_space());
    // }
    // if (dirt_query.solution_traj.size() != 0)
    // {
    //   vis_group->add_vis_infos(info_geometry_t::FULL_LINE, dirt_query.solution_traj, plant_name + "/body",
    //                            context.first->get_state_space(), "0xFF0000");
    // }
