...
if __name__ == "__main__":
    ...
    TM.pendulum_trajectory_ilqr([2,0])
    ...
    fout_roa = open(prx.out_path + "/py_pend_trajs_succ.txt", "w", buffering=2^10)
    
    # for th in np.arange(-np.pi, np.pi, 0.1):

    for th in tqdm(np.arange(-np.pi, np.pi, 0.1)):
        for thdot in np.arange(-2*np.pi, 2*np.pi, 0.1):
            l,r = TM.pendulum_trajectory_ilqr([th,thdot])
            TM.resulting_trajectory.to_file(prx.out_path + "/py_pend_track_trajs.txt", "a")
            fout_roa.write(str(TM.resulting_trajectory.front()) + " ")
            fout_roa.write(str(TM.resulting_trajectory.back()) + " ")
            fout_roa.write(str(r))
            fout_roa.write("\n")