...
int main(int argc, char* argv[])
{
  try
  {
    ...
    rastar_spec.cost_function = [](const trajectory_t& traj, const plan_t& plan) { return plan.duration(); };
    rastar_spec.distance_function = [](const space_point_t& s1, const space_point_t& s2) {
      return sqrt((s1->at(0) - s2->at(0)) * (s1->at(0) - s2->at(0)) +
             ...
             1.677;  // dividing by 1.0 since that is the max vel of the vehicle.
    };
