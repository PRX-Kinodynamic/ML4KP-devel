...
{
  auto params = param_loader("examples/roadmaps/trailer_car.yaml", argc, argv);
  ...
  context.first->get_state_space()->copy_point_from_vector(dirt_query.start_state,
                                                           params["/plant/start_state"].as<std::vector<double>>());
  context.first->get_state_space()->copy_point_from_vector(dirt_query.goal_state,
  ...
  dirt_query.goal_check = [&](space_point_t pt) 
  {
    return space_t::euclidean_2d(pt, dirt_query.goal_state, 0, 3) < dirt_query.goal_region_radius;
  };
  ...
  std::string body_name = params["/plant/name"].as<>() + "/car";
