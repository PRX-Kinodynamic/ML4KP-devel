...
int main(int argc, char* argv[])
{
  ...
  if (save_trajs_to_file)
  {
    ...
    std::cout << "trajs file: " << trajs_file_name << std::endl;
  }
  int traj_id = 0;
  ...
  // trajectory_t solution_traj(ss);
  space_point_t past_state = ss->make_point();
  ...
  auto compute_traj = [&](space_point_t& state) {
    ...
    solution_traj.clear();
    sg->propagate(state, lqr, checker, solution_traj);
    traj_duration = solution_traj.size() * simulation_step;
    ...
    past_state = solution_traj.front();
    for (auto state : solution_traj)
    {
      if (df(past_state, state) < 1)
      {
        fout_trajs << state << "\n";
      }
      else
  ...
  do
  {
    ...
    if (ss_dim > 2 && state->at(ss_dim - 1) != prev_last_dim)
    {
      ...
      std::ostringstream ss_file_id;
      ss_file_id << std::setw(5) << std::setfill('0') << file_id;
      // roa_file_name = lib_path + "out/" + plant_name +  "/lqr_" + ss_file_id.str() + "_" + horizon + "_" + tau_max +
      // "_roa.txt";
