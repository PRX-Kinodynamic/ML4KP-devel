        ...
        dirt_spec.node_expand = [&](dirt_roadmap_node_t* tree_node, std::vector<plan_t*>& plans, std::vector<trajectory_t*>& trajs, bool& override_child_extension)
        {
            ...
            if (tree_node->expand_num == 0)
            {
                ...
                if (roadmap_nodes.size() != 0)
                {
                    ...
                    // if (params["debug"].as<bool>()) std::cout << tree_node -> roadmap_cost_to_go << " " << nn -> get_node_cost() << std::endl;
                    if (tree_node -> roadmap_cost_to_go >= nn -> get_node_cost())
                    {
                        if (params["debug"].as<bool>()) std::cout << "Inside " << nn -> get_index() << " : " << ss -> print_point(nn->point,2) << std::endl;
                        ...
                        if (params["debug"].as<bool>()) std::cout << "Is node on path: " << is_node_on_path << std::endl;
                        ...
                        if (is_node_on_path && tree_node->greedy_expand)
                        {
                            ...
                            if (params["debug"].as<bool>()) std::cout << "Local goal index[a]: " << best_index << std::endl;
                        ...
                        else
                        {
                            ...
                            if (params["debug"].as<bool>()) std::cout << "Local goal index[b]: " << best_index << std::endl;
                        }
                    }
                    else
                    {
                        ...
                        {
                            best_index = rrr.get_next_local_goal(tree_node->point, controller_query, dirt_spec, controller, nn->get_successor());
                            if (params["debug"].as<bool>()) std::cout << "Local goal index[c]: " << best_index << std::endl;
                        }
                    }
                }
                ...
                if (best_index != -1)
                {
                    if (params["debug"].as<bool>()) std::cout << "Local goal: " << ss -> print_point(rrr.get_point(best_index), 4) << std::endl;
