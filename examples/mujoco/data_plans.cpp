...
int main()
{
  ...
  // discretize the goal position to cover the entire environment and run the planner for each goal position from a fixed start position
  for (int trial = 0; trial < num_trials; trial++)
  {
    ...
    while (true){
      selected_point[0] = uniform_random(-env_size[0] / 2 + 0.125, env_size[0] / 2 - 0.125);
      selected_point[1] = uniform_random(-env_size[1] / 2 + 0.125, env_size[1] / 2 - 0.125);
      // print repeats at which robot cartesian position
      double init_x = selected_point[0]; //uniform_random(-env_size[0] / 2 + 0.1, env_size[0] / 2 - 0.1);
      double init_y = selected_point[1]; //uniform_random(-env_size[1] / 2 + 0.1, env_size[1] / 2 - 0.1);
      sim->d->qpos[0] = init_x - robot_pos1[0];
      sim->d->qpos[1] = init_y - robot_pos1[1];
      mj_kinematics(sim->m, sim->d);
      if (! context.second->in_collision()){
        ...
        std::cout << sim->d->geom_xpos[id * 3] << " " << sim->d->geom_xpos[id * 3 + 1] << " "
                  << sim->d->geom_xpos[id * 3 + 2] << std::endl;
      }
    }
    while (true){
      // print repeats at which robot cartesian position
      double init_x = uniform_random(-env_size[0] / 2 + 0.1, env_size[0] / 2 - 0.1);
      double init_y = uniform_random(-env_size[1] / 2 + 0.1, env_size[1] / 2 - 0.1);
      sim->d->qpos[0] = init_x - robot_pos1[0];
      sim->d->qpos[1] = init_y - robot_pos1[1];
      // sim->d->qvel = 0.0;
      mj_kinematics(sim->m, sim->d);
      ...
      if (! context.second->in_collision()){
        ...
        std::cout << "robot in collision: " << std::endl;
        std::cout << sim->d->geom_xpos[id * 3] << " " << sim->d->geom_xpos[id * 3 + 1] << " "
                  << sim->d->geom_xpos[id * 3 + 2] << std::endl;
      }
    }
    ...
    std::vector<double> robot_position = {
        sim->d->geom_xpos[id * 3],
        sim->d->geom_xpos[id * 3 + 1],
        sim->d->geom_xpos[id * 3 + 2]
    };
    ...
    std::string trial_folder = output_folder_data + "trial_" + std::to_string(trial) + "/";
    ...
    try
    {
      ...
      // context.first->get_state_space()->copy_from(new_start_state);
  ...
  std::cout << "\nPlanning Summary: " << success_count << "/" << num_trials 
            << " trials succeeded (" 
            << (static_cast<double>(success_count) / num_trials * 100.0) 
