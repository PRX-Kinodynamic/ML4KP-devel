...
config = {
    ...
    "save_every": 1000,
    "start_epoch": 0,
    "mode": "test", # train, test, zmq
    # "device_id": 0,
    # "save_dir": "/common/users/dm1487/namo/diffusion/saved_models/only_object_mask/may1/fixed_start_fixed_goal_many_env_dinopeft_with_cond_inp_5_example",
    # "load_model_path": "/common/users/dm1487/namo/diffusion/saved_models/only_object_mask/may1/fixed_start_fixed_goal_many_env_dinopeft_with_cond_inp_5_example/best",
    ...
    "mode": "test", # train, test, zmq
    "device_id": 0,
    "save_dir": "/common/users/dm1487/namo/diffusion/saved_models/only_object_mask/may1/fixed_start_fixed_goal_many_env_dinopeft",
    "load_model_path": "/common/users/dm1487/namo/diffusion/saved_models/only_object_mask/may1/fixed_start_fixed_goal_many_env_dinopeft/epoch_450",
    "train_data_dir": ["/common/users/dm1487/namo_data/images/apr21/fixed_start_fixed_goal_many_env"],
    "cross_attention_dim": 384,
}
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return { "diffusion_input": image, "dino_input": npz_file['scene'].copy() }
...
class Runner:
    ...
    def _setup_accelerator(self, config):
        ...
        self.processor = AutoImageProcessor.from_pretrained('facebook/dinov2-small', use_fast=True)
        ...
        lora_config = LoraConfig(
            ...
            inference_mode= not (config["mode"] == "train"),
            lora_dropout=0.1,               # dropout probability for LoRA layers
            bias="none",                    # don't train biases
        )
        
        # self.dinoProjector = DinoProjector(self.dinov2, lora_config)
        ...
        # self.dinoProjector = nn.Sequential(
        #     nn.Linear(self.dinov2.config.hidden_size, self.config["cross_attention_dim"]),
        # )
        ...
        self.optimizer = AdamW(list(self.model.parameters()), lr=config["learning_rate"], weight_decay=config["weight_decay"])

        self.accelerator = Accelerator()
        self.model, self.dinov2, self.optimizer, self.train_dataloader = self.accelerator.prepare(self.model, self.dinov2, self.optimizer, self.train_dataloader)
    ...
    def train(self, start_epoch=0):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                dino_input = self.processor(images=dino_input, return_tensors="pt", do_rescale=False).to(self.device)
                dino_output = self.dinov2(**dino_input).last_hidden_state[:, 1:, :]
                ...
                #
                noise_pred = self.model(clean_images, timesteps, encoder_hidden_states=dino_output).sample
                
                # alphas_cumprod = self.noise_scheduler.alphas_cumprod[timesteps]
                # sigma2 = 1 - alphas_cumprod
                # weight = 1.0 / sigma2
                ...
                self.optimizer.step()
                #
                # noise_pred = self.model(clean_images, timesteps).sample
                # self.accelerator.backward(loss)
                # self.optimizer.step()
            ...
            print(f"Epoch {epoch+1}/{num_epochs} | loss={avg_loss:.4e} | lr={lr:.2e}")
    ...
    def save_model(self, epoch):
        if self.accelerator.is_main_process:
            ...
            # torch.save(self.dinoProjector.state_dict(), f"{self.config['save_dir']}/{epoch}/dino_projector.pth")
            self.dinov2.save_pretrained(f"{self.config['save_dir']}/{epoch}/dinov2")
            # self.dinov2.save_adapter(f"{self.config['save_dir']}/{epoch}/dinov2", adapter_name="default")
    ...
    def load_model(self, model_folder):
        ...
        # self.dinov2 = PeftModel.from_pretrained(self.dinov2, f"{model_folder}/dinov2", adapter_name="default")
        # self.dinov2.to(self.device)
        
        self.dinov2.load_adapter(f"{model_folder}/dinov2", adapter_name="default")
    ...
    def generate_image(self, scene_image=None, num_images=1):
        ...
        with torch.no_grad():
            ...
            dino_input = self.processor(images=dino_input, return_tensors="pt", do_rescale=False).to(self.device)
            dino_input['pixel_values'] = dino_input['pixel_values'].repeat(num_images, 1, 1, 1)
            dino_output = self.dinov2(**dino_input).last_hidden_state[:, 1:, :]
            # dino_input = self.dinoProjector(dino_input)
            for t in tqdm(reversed(range(0, self.noise_scheduler.config.num_train_timesteps))):
                if "dino_input" in inputs:
                    noise_pred = self.model(sample, t, encoder_hidden_states=dino_output).sample
                else:
...
if __name__ == "__main__":
    ...
    if config["mode"] == "train":
        ...
        folder_name = os.path.join("executables/v3/diffusion_v2/may1/peft_dino_many", config['env_config_name'])
