...
int main(int argc, char* argv[])
{
    try
    {
        ...
        rrr.compute_wavefront(g_nn,true);
        ...
        unsigned current = s_nn;
        std::vector<unsigned> path;
        while (current != g_nn)
        {
            path.push_back(current);
            current = rrr.get_vertex(current)->get_successor();
        }
        ...
        {
            std::string traj_fname = out_path + "/path_" + std::to_string(i) + ".txt";
            std::ofstream traj_file(traj_fname);
            traj_file << rrr.print_edge_traj(path[i],path[i+1],dirt_query,dirt_spec,controller);
            traj_file.close();
        }
    }
