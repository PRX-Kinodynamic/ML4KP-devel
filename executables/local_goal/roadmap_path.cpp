...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::vector<space_point_t> path;
        auto current = rrr.get_vertex(s_nn);
        while (current->get_index() != g_nn)
        {
            path.push_back(current->point);
            node_index_t next = current->get_successor();
            current = rrr.get_vertex(next);
        }
        path.push_back(current->point);
        ...
        ss -> copy_point(controller_query.start_state, path[0]);
        for (int i = 1; i < path.size(); i++)
        {
            ss -> copy_point(controller_query.goal_state, path[i]);
            std::cout << ss -> print_point(controller_query.start_state,2) << std::endl;
            std::cout << ss -> print_point(controller_query.goal_state,2) << std::endl;
            std::cout << std::endl;
            controller.fulfill_query(controller_query,dirt_spec);
            full_traj += controller_query.solution_traj;
            ss -> copy_point(controller_query.start_state, full_traj.back());
        }

        std::string body_name = params["/plant/name"].as<>() + "/" + params["/plant/vis_body"].as<>();
        three_js_group_t* vis_group = new three_js_group_t({plant},{obstacle_list});
        vis_group -> add_detailed_vis_infos(info_geometry_t::FULL_LINE, full_traj, body_name, ss);
        vis_group -> add_animation(full_traj, ss, dirt_query.start_state);
        vis_group->output_html("viz.html");
        delete vis_group;
