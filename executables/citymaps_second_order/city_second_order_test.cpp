...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        auto get_local_goal = [&](space_point_t& s, space_point_t& s2){
            at::Tensor heuristic_model_input = torch::zeros({1,8}, device);
            // at::Tensor heuristic_model_input = torch::zeros({1,5}, device);
            heuristic_model_input[0][0] = (s->at(0) + 25.6)/51.2;
            heuristic_model_input[0][1] = (s->at(1) + 25.6)/51.2;
            heuristic_model_input[0][2] = (s->at(2) + PRX_PI)/(2*PRX_PI);
            heuristic_model_input[0][3] = (s->at(3) + 0.7)/1.4;
            heuristic_model_input[0][4] = (s->at(4) + 0.7)/1.4;
            heuristic_model_input[0][5] = (s2->at(0) + 25.6)/51.2;
            heuristic_model_input[0][6] = (s2->at(1) + 25.6)/51.2;
            heuristic_model_input[0][7] = (s2->at(2) + PRX_PI)/(2*PRX_PI);

            std::vector<torch::jit::IValue> heuristic_model_inputs;
            heuristic_model_inputs.push_back(heuristic_model_input);

            at::Tensor heuristic_model_output = lg_model.forward(heuristic_model_inputs).toTensor();

            space_point_t lg = context.first->get_state_space()->make_point();
            lg -> at(0) = heuristic_model_output[0][0].item<double>()*51.2 - 25.6;
            lg -> at(1) = heuristic_model_output[0][1].item<double>()*51.2 - 25.6;
            lg -> at(2) = heuristic_model_output[0][2].item<double>()*2*PRX_PI - PRX_PI;
            if (space_t::euclidean_2d(lg, dirt_c2g_query.goal_state) < 2.0) lg = dirt_c2g_query.goal_state;
            return lg;
        };
        ...
        if (std::stoi(vm["controller"].as<std::string>()) == 1){
            dirt_c2g_spec.expand = [&](space_point_t& s, std::vector<plan_t*>& plans, std::vector<trajectory_t*>& trajs, int bn, bool blossom_expand)
            {
                ...
                if (blossom_expand)
                {
                    ...
                    for (int i = 0; i < bn; i++)
                    {
                        for (int j = 2; j < 5; j++) controller_input[i][j] = s -> at(j);
                        if (true){
                            ...
                            controller_input[i][5] = lg->at(0) - s->at(0);
                            controller_input[i][6] = lg->at(1) - s->at(1);
                            ...
                            controller_input[i][5] = dirt_c2g_query.goal_state -> at(0) - s->at(0);
                            controller_input[i][6] = dirt_c2g_query.goal_state -> at(1) - s->at(1);
                            controller_input[i][7] = dirt_c2g_query.goal_state -> at(2);
                        }   
                    }
        ...
        condition_check_t checker("iterations", 200); 
