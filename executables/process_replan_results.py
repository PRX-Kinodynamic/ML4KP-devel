...
exps_dir = os.environ["DIRTMP_PATH"] + "out/dynamic/benchmark/linear"
...
for i in range(len(dirs)):
    ...
    for j in range(num_trials):
        ...
        final_state = traj[-1]
        if((np.linalg.norm(final_state[:3] - goal) < goal_radius) and \
             (final_state[-1] == 1)):
            successes += 1
            solution_costs.append(simulation_step*(len(traj)-1))
    print(dirs[i]+","+f"{successes/num_trials:.2f}"
