...
{
	auto params = param_loader("executables/factor_graphs/sysid_multi.yaml", argc, argv);
 ...
 // 	std::cout << "[IN] traj back: " << traj_in.back() << std::endl;
	...
	graph.add(
		space_limit_factor_t(
			symbol_factory_t::create_symbol("param_symbol", 0),
			gtsam::noiseModel::Isotropic::Sigma(ps_dim, 1e0),
			ps
			)
		);
	auto dm = gtsam::noiseModel::Isotropic::Sigma(ss_dim, 1e0);
	auto add_traj = [&dm,&sg](const trajectory_t traj, const plan_t plan)
	{
		double t = 0;
		gtsam::NonlinearFactorGraph graph;
		...
		for (unsigned i = 0; i < traj.size(); ++i)
		{
			...
			{
				graph.add(
					propagation_factor_1_t(dm, 
						traj[i] -> to_vector(),
						traj[i+1] -> to_vector(),
						plan.at(static_cast<double>(t)) -> to_vector(),
						(Eigen::VectorXd(1) << simulation_step).finished(),
						symbol_factory_t::create_symbol("param_symbol", 0),
						sg)
					);
			}
	...
	// std::vector<std::string> traj_files = params["data_dir"].as<>() + params["traj_files_in"].as<>();
	std::vector<std::string> traj_files = params["traj_files_in"].as<std::vector<std::string>>();
	std::vector<std::string> plan_files = params["plan_files_in"].as<std::vector<std::string>>();

	prx_assert(traj_files.size() == plan_files.size(), "mismatch number of traj and plan files!");
	...
	for (int i = 0; i < traj_files.size(); ++i)
	{
		plan_i.clear();
		traj_i.clear();
		traj_i.from_file(params["data_dir"].as<>() + traj_files[i]);
		plan_i.from_file(params["data_dir"].as<>() + plan_files[i]);

		graph.add(add_traj(traj_i, plan_i));
	}
