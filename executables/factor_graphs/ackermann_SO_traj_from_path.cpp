...
{
  auto params = param_loader("executables/factor_graphs/ackermann_SO_traj_from_path.yaml", argc, argv);
  ...
  while (reader.has_next_line())
  {
    ...
    traj.copy_onto_back(Eigen::Vector<double, 5>(x, y, theta, 0, 0));
  ...
  auto dm = gtsam::noiseModel::Diagonal::Sigmas(Eigen::Vector<double, 5>(1e-3, 1e-3, 1e0, 1e0, 1e0));
  ...
  for (int i = 0; i < 1; ++i)
  {
    ...
    for (std::size_t i = 0; i < total_states - 1; ++i)
    {
      ...
      graph.add(
          quadratic_cost_factor_t<X_DIM>(state_symbol, traj[i]->vector<>(), Eigen::Matrix<double, 5, 5>::Ones(), 1e0));
