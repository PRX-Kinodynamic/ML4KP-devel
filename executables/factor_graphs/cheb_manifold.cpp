...
{
  using SE3Manifold = manifold_evaluation_t<gtsam::Chebyshev2, gtsam::Pose3>;
  using SE3CloseFactor = manifold_close_factor_t<gtsam::Chebyshev2, gtsam::Pose3>;
  using Chebyshev2ManifoldFunctor = gtsam::Chebyshev2::ManifoldEvaluationFunctor<gtsam::Pose3>;
  // using Chebyshev2LieAlgebraFunction = gtsam::Chebyshev2::VectorEvaluationFunctor<6>;
  using Chebyshev2LieAlgebraFunction = gtsam::Chebyshev2::VectorDerivativeFunctor<6>;
  // using CloseDerivFactor = close_deriv_factor_t<gtsam::Chebyshev2, gtsam::Pose3>;
  ...
  const double a{ 0.0 };
  const double total_time{ params["Ttotal"].as<double>() };
  const double b{ total_time };
  // const double b{ data.size() * total_time };
  const std::size_t N{ params["N"].as<std::size_t>() };

  const double total_points{ static_cast<double>(data.size()) };
  double ti{ 0.0 };

  const double step{ total_time / data.size() };
  PRX_DBG_VARS(a, b, N, step, total_points);
  ...
  // graph.addPrior(gtsam::Symbol('x', 0), );
  ...
  // {
  //   const gtsam::Chebyshev2::VectorEvaluationFunctor<3> fx(N, ti, a, b);
  //   std::cout << ti << " " << fx(param_mat).transpose() << "\n";
  // }

  std::ofstream ofs(prx::out_path + "/chebyshev_manifold.txt");
  ...
  Eigen::MatrixXd Dm{ gtsam::Chebyshev2::DifferentiationMatrix(param_mat.rows()) };
  // PRX_DBG_VARS(Dm)
  PRX_DBG_VARS(Dm.rows(), Dm.cols());
  Eigen::MatrixXd derivative{ Dm * param_mat.matrix() };
  PRX_DBG_VARS(derivative.rows(), derivative.cols());
  // gtsam::ParameterMatrix<6> derivativeCoefficients{ derivative };  // = Eigen::MatrixXd(Dn * param_mat.matrix());

  Eigen::VectorXd chev_points{ gtsam::Chebyshev2::Points(N) };

  // Start at a + dt for now due to numerical error in numerical deriv
  ti = a + step;
  ofs_poly << "# idx x y z qw qx qy qz xd yd zd Rxd Ryd Rzd \n";
  for (int i = 1; i < data.size() - 1; ++i)
  {
    // PRX_DBG_VARS(ti, a, b);
    Eigen::MatrixXd Hfx;

    const Chebyshev2LieAlgebraFunction fla(N, ti, a, b);
    const Chebyshev2ManifoldFunctor fx(N, ti, a, b);
    const gtsam::Pose3 in_pose{ data[i] };
    const gtsam::Pose3 f_pose{ fx(param_mat, Hfx) };

    // gtsam::Chebyshev2::VectorEvaluationFunctor<6> dfdx(N, ti, a, b);
    // const Eigen::VectorXd xi{ dfdx(param_mat) };
    // std::cout << "----- ----- ------ ------\n";

    std::function<gtsam::Pose3(const double&)> proxy = [&](const double& xt) {
      // PRX_DBG_VARS(N, xt, a, b)
      const Chebyshev2ManifoldFunctor fx(N, xt, a, b);
      gtsam::Pose3 res{ fx(param_mat) };
      // pose3_to_stream(std::cout, res);
      // std::cout << "\n";
      return res;
      // return gtsam::Chebyshev2::VectorEvaluationFunctor<6>(N, xt, a, b)(param_mat);
    };
    Eigen::MatrixXd x_tau{ gtsam::numericalDerivative11<gtsam::Pose3, double>(proxy, ti, step * step) };
    // std::cout << xi.transpose() << "\n";
    Eigen::VectorXd xi{ gtsam::Pose3::Logmap(f_pose * gtsam::Pose3::Expmap(x_tau) * f_pose.inverse()) };

    // const Eigen::VectorXd xi{ gtsam::Pose3::ChartAtOrigin::Local(f_pose) };
    // gtsam::Weights dWeights{ gtsam::Chebyshev2::DerivativeWeights(N, ti, a, b) };
    // PRX_DBG_VARS(tang)
    // PRX_DBG_VARS(dWeights)
    // Eigen::VectorXd fvals{ gtsam::Chebyshev2::Points(N) };
    // PRX_DBG_VARS(fvals)

    // const Eigen::VectorXd xi{ dWeights * fvals };
    // gtsam::Chebyshev2::VectorDerivativeFunctor<6> dfdx(N, ti, a, b);
    // Vector derivative_at_points = D6 * f3_at_6points;

    // gtsam::Chebyshev2::VectorEvaluationFunctor<6> dfdx(N, ti, a, b);
    // gtsam::Chebyshev2::VectorEvaluationFunctor<6> dfdx(N, ti, a, b);

    // gtsam::Weights wi{ gtsam::Chebyshev2::CalculateWeights(N, ti, a, b) };
    // PRX_DBG_VARS(wi.rows(), wi.cols());
    // Eigen::MatrixXd dev{ derivative * wi.transpose() };
    // PRX_DBG_VARS(dev);
    // gtsam::Chebyshev2::VectorEvaluationFunctor<6> dfdx(N, ti, a, b);
    ...
    // gtsam::ParameterMatrix<6> pd(dev);
    // PRX_DBG_VARS(pd);
    // auto xi = dfdx(pd);
  ...
  const Chebyshev2ManifoldFunctor fa(N, a, a, b);
  const Chebyshev2ManifoldFunctor fb(N, b, a, b);
