...
using wheel_frictions_t = Eigen::Vector<double, 4>;
...
Eigen::Vector3d wheel_transform(double x, double y, double theta, int wheel_num)  // no-lint
{
  ...
  auto v = rot * xy;
  return Eigen::Vector3d(v[0], v[1], 0);  // At least for now
};
...
int main(int argc, char* argv[])
{
  ...
  wheel_frictions_t friction_params{ wheel_frictions_t::Zero() };
  ...
  std::function<void()> real_world = [&]()  // no-lint
  {
    ...
    Eigen::Vector3d w1{ wheel_transform(x, y, th, 0) };
    Eigen::Vector3d w2{ wheel_transform(x, y, th, 1) };
    Eigen::Vector3d w3{ wheel_transform(x, y, th, 2) };
    Eigen::Vector3d w4{ wheel_transform(x, y, th, 3) };

    friction_params[0] = friction_2_at(w1[0], w1[1], real_thetas_vector, frictions_grid)[0];
    friction_params[1] = friction_2_at(w2[0], w2[1], real_thetas_vector, frictions_grid)[0];
    friction_params[2] = friction_2_at(w3[0], w3[1], real_thetas_vector, frictions_grid)[0];
    friction_params[3] = friction_2_at(w4[0], w4[1], real_thetas_vector, frictions_grid)[0];

    if (x < 0.0 || x_max < x || y < 0.0 || y_max < y)
    {
      friction_params = wheel_frictions_t::Ones();
    }
    if (write_to_file)
    {
  ...
  std::function<void()> recovered_world = [&]()  // no-lint
  {
    ...
    friction_params[0] = friction_at(w1[0], w1[1], real_thetas_vector, frictions_grid)[0];
    friction_params[1] = friction_at(w2[0], w2[1], real_thetas_vector, frictions_grid)[0];
    friction_params[2] = friction_at(w3[0], w3[1], real_thetas_vector, frictions_grid)[0];
    friction_params[3] = friction_at(w4[0], w4[1], real_thetas_vector, frictions_grid)[0];
  ...
  for (int i = 0; i < num_trajs; ++i)
  {
    ...
    for (unsigned xi = 0; xi < traj_real.size(); xi += increment)
    {
      if (xi + increment < traj_real.size())
      {
        ...
        auto weights_symbol_0 = symbol_factory_t::create_symbol("weight_symbol", 0, xi);
        auto weights_symbol_1 = symbol_factory_t::create_symbol("weight_symbol", 1, xi);
        auto weights_symbol_2 = symbol_factory_t::create_symbol("weight_symbol", 2, xi);
        auto weights_symbol_3 = symbol_factory_t::create_symbol("weight_symbol", 3, xi);
        ...
        trajectory_values.insert(param_symbol, (Eigen::VectorXd(4) << 1, 1, 1, 1).finished());

        // basis_vector_t compute_weights_vector(ThetaPosGrid& pos_grid, const state_t& theta_i_pos)
        auto state_vec = traj_real[xi]->vector<state_t>();
        const basis_vector_t weight_w0{ compute_weights_vector(
            frictions_grid, wheel_transform(state_vec[0], state_vec[1], state_vec[2], 0)) };
        const basis_vector_t weight_w1{ compute_weights_vector(
            frictions_grid, wheel_transform(state_vec[0], state_vec[1], state_vec[2], 1)) };
        const basis_vector_t weight_w2{ compute_weights_vector(
        ...
        weights_graph.addPrior(weights_symbol_3, weight_w3, weight_nm);
