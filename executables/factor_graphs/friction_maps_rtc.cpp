...
{
  auto params = param_loader("executables/factor_graphs/friction_map_rtc.yaml", argc, argv);
  ...
  std::vector<std::vector<double>> goals = { { uniform_random() * x_max, uniform_random() * y_max, 0 } };
  ...
  const std::size_t increment = static_cast<std::size_t>(frequency / simulation_step);
  ...
  condition_check_t checker("iterations", increment);
  ...
  space_point_t state_k0 = ss->make_point();
  const int initial_goal{ params["initial_goal"].as<int>() };
  ss->copy(start_state, goals[0]);
  for (int i = initial_goal; i < num_trajs; ++i)
  {
    ...
    std::size_t k{ 0 };
    plan_t executed_plan(cs);
    // initial propagation
    sg->propagate(start_state, omnibot_controller, checker, traj_real);
    ss->copy(state_k0, traj_real.back());
    const plan_t plan_0{ *(omnibot_controller->get_plan()) };

    x1 = traj_real[0]->vector<>();
    // x1 = traj_real.back()->vector<>();
    u1 = plan_0[0].control->vector<>();
    t1 = (Eigen::VectorXd(1) << plan_0[0].duration * increment).finished();
    check_goal_reached.reset();
    do
    {
      omnibot_controller->get_plan()->clear();
      checker.reset();
      ...
      sg->propagate(state_k0, omnibot_controller, checker, traj_real);
      ss->copy(state_k0, traj_real.back());
      ...
      // if (traj_real.size() < increment * 2)
      // {
      //   continue;
      // }

      const std::size_t k0{ k / increment };
      const std::size_t k1{ k0 + 1 };
      const std::size_t k2{ k0 + 2 };

      x0 = x1;
      x1 = traj_real[0]->vector<>();
      x2 = traj_real.back()->vector<>();

      u0 = u1;
      u1 = plan[0].control->vector<>();

      t0 = t1;
      t1 = (Eigen::VectorXd(1) << plan[0].duration * increment).finished();

      // PRX_DEBUG_VAR_3(x0.transpose(), x1.transpose(), x2.transpose());
      if (fg_grid(x0[0], x0[1]) != fg_grid(x1[0], x1[1]) || fg_grid(x1[0], x1[1]) != fg_grid(x2[0], x2[1]))
      {
        continue;
      }

      auto state_symbol_0 = symbol_factory_t::create_symbol("state_symbol", i, k0);
      auto state_symbol_1 = symbol_factory_t::create_symbol("state_symbol", i, k1);
      auto state_symbol_2 = symbol_factory_t::create_symbol("state_symbol", i, k2);
      auto control_symbol_0 = symbol_factory_t::create_symbol("control_symbol", i, k0);
      auto control_symbol_1 = symbol_factory_t::create_symbol("control_symbol", i, k1);
      auto time_symbol_0 = symbol_factory_t::create_symbol("time_symbol", i, k0);
      auto time_symbol_1 = symbol_factory_t::create_symbol("time_symbol", i, k1);
      auto param_symbol = symbol_factory_t::create_symbol("param_symbol", fg_grid(x0[0], x0[1]));

      graph_trajs.addPrior(state_symbol_0, x0, x_sigma);
      graph_trajs.addPrior(state_symbol_1, x1, x_sigma);
      graph_trajs.addPrior(state_symbol_2, x2, x_sigma);
      graph_trajs.addPrior(control_symbol_0, u0, cs_dm);
      graph_trajs.addPrior(control_symbol_1, u1, cs_dm);
      graph_trajs.addPrior(time_symbol_0, t0, t_dm);
      graph_trajs.addPrior(time_symbol_1, t1, t_dm);

      init_vals.insert_or_assign(state_symbol_0, x0);
      init_vals.insert_or_assign(state_symbol_1, x1);
      init_vals.insert_or_assign(state_symbol_2, x2);
      init_vals.insert_or_assign(control_symbol_0, u0);
      init_vals.insert_or_assign(control_symbol_1, u1);
      init_vals.insert_or_assign(time_symbol_0, t0);
      init_vals.insert_or_assign(time_symbol_1, t1);

      // PRX_DEBUG_VAR_2(k0, k2);
      // PRX_DEBUG_VAR_2(prx::key_formatter(state_symbol_0), state_0.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(state_symbol_1), state_1.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(state_symbol_2), state_2.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(control_symbol_0), u0.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(control_symbol_1), u1.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(time_symbol_0), t0.transpose());
      // PRX_DEBUG_VAR_2(prx::key_formatter(time_symbol_1), t1.transpose());

      graph_trajs.add(propagation_factor_5_t<3, 4, 1>(state_symbol_0, state_symbol_1, control_symbol_0, time_symbol_0,
                                                      param_symbol, dm, sg));
      graph_trajs.add(propagation_factor_5_t<3, 4, 1>(state_symbol_1, state_symbol_2, control_symbol_1, time_symbol_1,
                                                      param_symbol, dm, sg));

      // graph_trajs.add(propagation_factor_1_t(dm, x0, x1, u0, t0, param_symbol, sg));

      // graph_trajs.print("graph_trajs:\n", prx::key_formatter);
      ...
      theta_priors[param_symbol] = marginals.marginalCovariance(param_symbol);
