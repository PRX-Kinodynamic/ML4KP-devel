...
gtsam::Key poly_symbol(const int rod)
{
  return SF::create_hashed_symbol("p^{", rod, "}");
}

void read_data(const std::string& filename, std::vector<std::vector<double>>& data)
{
  ...
  while (reader.has_next_line())
  {
    ...
    const double idx{ convert_to<double>(line[1]) };
    const double id{ convert_to<double>(line[2]) };

    curr.emplace_back(t);
    curr.emplace_back(idx);
    curr.emplace_back(id);
...
int main(int argc, char* argv[])
{
  ...
  std::vector<std::vector<double>> data_red;
  read_data(filename_red, data_red);

  const double a{ 0.0 };
  const double b{ data_red.back()[0] };
