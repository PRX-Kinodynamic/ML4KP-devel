...
gtsam::GaussianBayesNet::shared_ptr BlockEliminateSequential(gtsam::GaussianFactorGraph graph,
                                                             const std::vector<gtsam::Ordering>& ordering)
{
  // setup
  gtsam::VariableIndex variableIndex(graph);  // maps keys to factor indices
  auto bn = boost::make_shared<gtsam::GaussianBayesNet>();
...
int main(int argc, char* argv[])
{
  ...
  aorrt_query_t aorrt_query(ss, context.first->get_control_space());
  ...
  // A << 3, 0, 0, 0, 2, 1, 0, 0, 1, 0, 1, 0, 1, 1, 1, 1;
  // Eigen::Vector<double, 4> b;
  // b << 4, 2, 4, 2;
  // auto x = A.triangularView<Eigen::Lower>().solve(b);
  // PRX_DEBUG_VAR_3(A, b, x);
  // PRX_DEBUG_VAR_1(A * x);
  // ilqr_elimination_tree->print("EliminationTree: ", prx::key_formatter);
