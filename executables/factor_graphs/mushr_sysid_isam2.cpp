...
void add_observations(gtsam::NonlinearFactorGraph& graph, gtsam::Values& values, const std::string& filename,
                      const prx::trajectory_t& traj, const prx::plan_t& plan, gtsam::ISAM2& isam2)
{
  ...
  while (reader.has_next_line())
  {
    ...
    if (line.size() > 0)
    {
      ...
      PRX_DEBUG_VAR_1(xs.back().transpose());
    }
  }
  double t_prev{ 0.0 };
  double t_now{ 0.0 };
  ...
  const Eigen::Vector3d x0{ Vec(traj.at(t_accum)).head(3) };
  values.insert(k_X(0), x0);
  ...
  for (int i = 0; i < ts.size() - 1; ++i)
  {
    ...
    // PRX_DEBUG_VAR_1(xt_next.transpose());
    // PRX_DEBUG_VAR_1(xdt.transpose());
    // PRX_DEBUG_VAR_1(ut.transpose());
    // PRX_DEBUG_VAR_1(ubar.transpose());
    // ubar = mushr_ub_u_xdot_t::predict(ut, xdt, params_dv, config.length);
    values.insert(k_X(ti + 1), xt_next);
    ...
    // auto results = isam2.calculateEstimate();
    graph.resize(0);
    values.clear();
    ti++;
    t_accum += dt;
  }
  auto results = isam2.calculateBestEstimate();
  PRX_DEBUG_VAR_1(graph.error(results));
  for (int ti = 0; ti < ts.size() - 1; ++ti)
  {
    const Eigen::Vector3d xi{ results.at<Eigen::Vector3d>(k_X(ti)) };
    PRX_DEBUG_VAR_1(xi.transpose());
  }
  // t_accum += dt;
  // t_prev = t_now;
  // first = false;
...
int main(int argc, char* argv[])
{
  ...
  parameters.keyFormatter = prx::fg::symbol_factory_t::formatter;
  gtsam::ISAM2 isam2(parameters);
  add_observations(graph, values, params["observations"].as<>(), traj, plan, isam2);
  ...
  // }; PRX_DEBUG_VAR_1(params_out); ps->copy_from(params_out); prx::trajectory_t res_traj{ ss };
