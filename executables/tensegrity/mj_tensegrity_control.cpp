...
int main(int argc, char** argv)
{
  ...
  std::vector<double> stable_lengths = { 0, 32, 32, 32, 32, 37, 32, 45, 37, 32, 37, 37, 32 };

  using Ctrl = Eigen::Vector<double, 12>;
  Ctrl ctrl_v_stable(37, 37, 37, 37, 32, 32, 32, 32, 32, 32, 32, 32);
  Ctrl ctrl_v(37, 37, 37, 45, 32, 32, 32, 32, 32, 32, 32, 32);
  Ctrl ctrl_d(Ctrl::Zero());
  plan.copy_onto_back(ctrl_v_stable, ti);
  // plan.copy_onto_back(ctrl_v_stable, ti);

  cs->copy(ctrl0, ctrl_v_stable);
  ...
  while (reader.has_next_line())
  {
    ...
    if (prev_time > 0)
    {
      ...
      cs->copy(ctrl1, ctrl_v);
      PRX_DEBUG_VAR_1(ctrl0);
      PRX_DEBUG_VAR_1(ctrl1);
      for (double inter = 0; inter < 1; inter += prx::simulation_step / ti)
      {
        cs->interpolate(ctrl0, ctrl1, inter, ctrl_ti);
        plan.copy_onto_back(ctrl_ti, prx::simulation_step);
