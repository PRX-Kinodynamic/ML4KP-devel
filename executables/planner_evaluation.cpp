...
int main(int argc, char* argv[])
{
  try
  {
    ...
    auto planner_wrapper =
        create_planner_wrapper("planning_context", context, param_loader(params["planner_parameters"]));
    ...
    for (int i = 0; i < stats_runs; i++)
    {
      ...
      std::string full_filename = lib_path + params["data_output_folder"].as<std::string>() +
                                  params["planner_parameters"]["planner_name"].as<std::string>() + "_" +
                                  std::to_string(i) + ".txt";
      ...
      if (get_visualization)
      {
        ...
        {
          vis_group->add_vis_infos(info_geometry_t::LINE, planner_wrapper->planner_query->solution_traj, vis_body,
                                   context.first->get_state_space(), "0x00ffff");
          double timestamp = 0;
