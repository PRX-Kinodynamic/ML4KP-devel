                  ...
                  prx::rrt_star_specification_t& spec)
{
  ...
  if (!added)
  {
      ...
      std::cout << "Suggested state not added." << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  const std::string out_dir_updated{ prx::out_path + "peg_in_hole/updated" };
  const std::string out_dir_updated_traj{ prx::out_path + "peg_in_hole/updated" };
  ...
  std::cout << "out_dir_updated_traj: " << out_dir_updated_traj << std::endl;
  ...
  if (grow_tree)
  {
    ...
    for (const auto& eigenState : suggestedStates)
    {
      ...
      /*
      auto pqp_dists = default_obstacle_distance_function(eigenVector, ss, context.second);
      ...
      std::cout << "In collision? " << rrt_star_spec.valid_state(eigenVector) << " ; Dist = " << min_dist << std::endl;
      */
  ...
  vis_group->add_vis_infos(info_geometry_t::SPHERE, { Vec(rrt_star_query.goal_state).head(3) }, "0xffff00",rrt_star_query.goal_region_radius);
