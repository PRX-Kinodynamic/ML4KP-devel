...
void pyprx_planning_planner_functions_py()
{
    ...
    class_<prx::valid_trajectory_t>("valid_trajectory")
        ...
        .def("wrap", &create_function<prx::valid_trajectory_t, bool, prx::trajectory_t>).staticmethod("wrap")
        ;

    class_<prx::sample_state_t>("sample_state")
        .def("__call__", &prx::sample_state_t::operator() )
        .def("wrap", &create_function<prx::sample_state_t, void, prx::space_point_t&>).staticmethod("wrap")
        ;
