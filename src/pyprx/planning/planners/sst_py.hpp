...
void bindings()
{
  class_<prx::sst_node_t, bases<prx::rrt_node_t>>("sst_node", init<>())
      ...
      .add_property("witness_index", &get_sst_node_t_witness_index<prx::node_index_t>,
                    &get_sst_node_t_witness_index<prx::node_index_t>)
      // Comment to force ; to the next one
      ;

  class_<prx::witness_node_t, bases<prx::tree_node_t>>("witness_node", init<>())
      .add_property("rep_index", &get_witness_node_t_rep_index<prx::node_index_t>,
                    &get_witness_node_t_rep_index<prx::node_index_t>)
      .add_property("set", &get_witness_node_t_set<bool>, &get_witness_node_t_set<bool>)
      // Comment to force ; to the next one
      ;

  class_<prx::sst_specification_t, std::shared_ptr<prx::sst_specification_t>, bases<prx::rrt_specification_t>>(
      "sst_specification", init<std::shared_ptr<prx::system_group_t>, std::shared_ptr<prx::collision_group_t>>())
      .add_property("delta_near", &get_sst_specification_t_delta_near<double>,
                    &set_sst_specification_t_delta_near<double>)
      .add_property("delta_drain", &get_sst_specification_t_delta_drain<double>,
                    &set_sst_specification_t_delta_drain<double>)
      // Comment to force ; to the next one
      ;

  class_<prx::sst_query_t, std::shared_ptr<prx::sst_query_t>, bases<prx::rrt_query_t>>("sst_query", no_init)
      .def("__init__", make_constructor(&init_as_ptr<prx::sst_query_t, prx::space_t*, prx::space_t*>,
  ...
  class_<prx::sst_t, bases<prx::rrt_t>>("sst", no_init)
      .def("__init__",
           make_constructor(&init_as_ptr<prx::sst_t, std::string>, default_call_policies(), (arg("new_name"))))
