...
static inline void IDX_CHECK(Index i, Index MAX)
{
  if (i < 0 || i >= MAX)
  {
    PyErr_SetString(PyExc_IndexError, ("Index " + boost::lexical_cast<std::string>(i) + " out of range 0.." +
...
static inline void IDX2_CHECKED_TUPLE_INTS(tuple tuple, const Index max2[2], Index arr2[2])
{
  ...
  {
    extract<Index> val(tuple[_i]);
    if (!val.check())
    {
      PyErr_SetString(PyExc_ValueError,
                      ("Unable to convert " + boost::lexical_cast<std::string>(_i) + "-th index to integer.").c_str());
      throw_error_already_set();
    }
    Index v = val();
    IDX_CHECK(v, max2[_i]);
...
template <typename T>
static Scalar get_item(const T& a, tuple _idx)
{
  Index idx[2];
...
template <typename T>
static void set_item(T& a, tuple _idx, const Scalar& value)
{
  Index idx[2];
  Index mx[2] = { a.rows(), a.cols() };
  IDX2_CHECKED_TUPLE_INTS(_idx, mx, idx);
...
template <typename T>
static void set_item_v(T& a, Index _idx, const Scalar& value)
{
  a[_idx] = value;
}
template <typename T>
static Scalar get_item_v(const T& a, Index _idx)
{
  return a[_idx];
}
...
static R diagonal(const T& m)
{
  return m.diagonal();
}

template <typename T>
static T __imul__(T& a, const T& b)
{
  a *= b;
  return a;
};
...
template <typename T, typename R>
static R __mul__vec(const T& m, const R& v)
{
  return m * v;
}
// float matrices only
...
template <typename T>
static T __div__(const T& a, const T& b)
{
  return a / b;
}
...
{
  return T::Ones(rows, cols);
}
template <typename T>
static T Zero(Index rows, Index cols)
{
  return T::Zero(rows, cols);
}
template <typename T>
static T Random(Index rows, Index cols)
{
  return T::Random(rows, cols);
}
template <typename T>
static T Identity(Index rows, Index cols)
{
  return T::Identity(rows, cols);
}

template <typename T>
static T Zero_1d(Index size)
{
  return T::Zero(size);
}
...
static prx::quaternion_t* fromAngleAxis(const Scalar& angle, const prx::vector_t& axis)
{
  prx::quaternion_t* ret = new prx::quaternion_t(AngleAxisT(angle, axis));
  ret->normalize();
  return ret;
}
static prx::quaternion_t* fromTwoVectors(const prx::vector_t& u, const prx::vector_t& v)
{
  prx::quaternion_t* q(new prx::quaternion_t);
...
void bindings()
{
  ...
  class_<Eigen::MatrixXd>("matrix", init<Eigen::MatrixXd>())
      ...
      // )) .def("__init__", make_constructor(&init_as_ptr<Eigen::Matrix4d,double,double,double,double>,
      // default_call_policies() )) .def("__setitem__", &set_matrix_item< Eigen::MatrixXd >)
      ...
      .def("__imul__", &__imul__<Eigen::MatrixXd>)
      // // matrix*vector product
      .def("__mul__", &__mul__vec<Eigen::MatrixXd, Eigen::VectorXd>)
      .def("__rmul__", &__mul__vec<Eigen::MatrixXd, Eigen::VectorXd>)
      .def("Zero", &Zero<Eigen::MatrixXd>, (arg("rows"), arg("cols")), "Create zero matrix of given dimensions")
      .staticmethod("Zero")
      .def("Ones", &Ones<Eigen::MatrixXd>, (arg("rows"), arg("cols")),
           "Create matrix of given dimensions where all elements are set to 1.")
      .staticmethod("Ones")
      .def("Random", &Random<Eigen::MatrixXd>, (arg("rows"), arg("cols")),
           "Create matrix with given dimensions where all elements are set to number between 0 and 1 "
           "(uniformly-distributed).")
      .staticmethod("Random")
      .def("Identity", &Identity<Eigen::MatrixXd>, (arg("rows"), arg("cols")),
           "Create identity matrix with given rows anc columns.")
      .staticmethod("Identity")
      // .def("__setitem__",&prx::matrix_t::set_row).def("__getitem__",&prx::matrix_t::get_row)
      // .def("__setitem__",&prx::matrix_t::set_item< prx::matrix_t >).def("__getitem__",&prx::matrix_t::get_item<
  ...
  class_<prx::quaternion_t>("quaternion")
      ...
      .def(init<Scalar, Scalar, Scalar, Scalar>((arg("w"), arg("x"), arg("y"), arg("z")),
