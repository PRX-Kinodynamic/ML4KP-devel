...
namespace prx
{
    ...
    void mujoco_plant_t::initialize(std::shared_ptr<mujoco_simulator_t> sim)
    {
        ...
        for (int i = 0; i < sim -> m -> nq; i++) state_memory.push_back(new double);
        for (int i = 0; i < sim -> m -> nv; i++) state_memory.push_back(new double);
        for (int i = 0; i < sim -> m -> nu; i++) control_memory.push_back(new double);
