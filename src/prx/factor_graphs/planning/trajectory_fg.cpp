...
trajectory_fg_t::trajectory_fg_t(system_ptr_t _sys_ptr)
{
  ...
  dynamics_model =  // nullptr;
      gtsam::noiseModel::Isotropic::Sigma(plant_ptr->get_state_space()->get_dimension(), 1e0);
  control_model =  // nullptr;
      gtsam::noiseModel::Isotropic::Sigma(plant_ptr->get_control_space()->get_dimension(),
...
gtsam::NonlinearFactorGraph trajectory_fg_t::limits_factors(const int t, const int num_steps) const
{
  ...
  graph.add(space_limit_factor_t(sx_t, dynamics_model, ss));

  if (t < num_steps)
  {
    graph.add(space_limit_factor_t(su_t, control_model, cs));
  }
...
gtsam::NonlinearFactorGraph trajectory_fg_t::add_state_propagation_factor(const int t)
{
  ...
  {
    graph.add(state_propagation_factor_t(nullptr, x0_key, x1_key, u1_key, i, plant_ptr));
  }
...
gtsam::NonlinearFactorGraph trajectory_fg_t::add_bang_bang_factor(const int t)
{
  ...
  graph.add(bang_bang_factor_t(control_model, u_key, plant_ptr, t));
...
gtsam::NonlinearFactorGraph trajectory_fg_t::add_propagation_factor(const int t, const int prop_type)
{
  ...
  if (prop_type == 3)
  {
    graph.add(propagation_factor_t(dynamics_model, x0_key, x1_key, u1_key, plant_ptr));
    // graph.add(
  ...
  else if (prop_type == 4)
  {
    ...
    graph.add(propagation_factor_4_t(dynamics_model, x0_key, x1_key, u1_key, t01_key, plant_ptr));
  }
  else if (prop_type == 5)
  {
...
gtsam::NonlinearFactorGraph trajectory_fg_t::add_energy_factors(const int t, goal_factor_params_t params)
{
  ...
  graph.add(kinetic_energy_factor_t(control_model, xi, plant_ptr, goal_state, t, params));

  graph.add(potential_energy_factor_t(control_model, xi, plant_ptr, goal_state, params.T - t, params));
...
// const tot_steps, const double theta, Eigen::VectorXd& error_scale)
{
  ...
  graph.add(goal_distance_factor_t(dynamics_model, xi_f, plant_ptr, t, goal_state, _params));

  return graph;
}

gtsam::NonlinearFactorGraph trajectory_fg_t::get_smoothing_fg(const trajectory_fg_params_t& _params)
{
  gtsam::NonlinearFactorGraph graph;
  int num_steps = _params.num_steps;

  // X_0
  graph.add(gtsam::NonlinearEquality<Eigen::VectorXd>(prx_symbol_t::state_symbol(0), initial_state));
  // X_N
  // graph.add(
  ...
  for (int t = 0; t < num_steps + 1; t++)
  {
    ...
    if (_params.use_goal_factors)
      graph.add(add_goal_distance_factor(t, _params.goal_factor_params));
  }

  return graph;
}

gtsam::NonlinearFactorGraph trajectory_fg_t::get_recovering_ctrls_fg(const trajectory_t& traj,
                                                                     ...
                                                                     const trajectory_fg_params_t& _params)
{
  ...
  for (unsigned i = 0; i < traj.size(); ++i)
  {
    ...
    if (i < traj.size() - 1)
    {
      graph.add(space_limit_factor_t(symbol_factory_t::create_symbol("control_symbol", i), control_model,
                                     plant_ptr->get_control_space()));
      graph.add(
          // auto t01_key = prx_symbol_t::time_symbol(t);
          propagation_factor_1_t(dynamics_model, traj[i]->vector<>(), traj[i + 1]->vector<>(),
                                 symbol_factory_t::create_symbol("control_symbol", i),
                                 (Eigen::VectorXd(1) << 1).finished(), Eigen::VectorXd::Zero(1), sg));
      // graph.add(
      // 	gtsam::NonlinearEquality<Eigen::VectorXd>(
...
gtsam::NonlinearFactorGraph trajectory_fg_t::get_fg(const trajectory_fg_params_t& _params)
{
  gtsam::NonlinearFactorGraph graph;
  int num_steps = _params.num_steps;
  ...
  if (_params.goal_state_as_prior)
  {
    ...
    // 	dynamics_model
    // 	);
    graph.add(gtsam::NonlinearEquality<Eigen::VectorXd>(prx_symbol_t::state_symbol(num_steps), goal_state, 0.5));
  }
  ...
  for (int t = 0; t < num_steps + 1; t++)
  {
    ...
    // if (_params.use_goal_factors && t == num_steps -1) graph.add(add_goal_distance_factor(t,
    // _params.goal_factor_params));
    if (_params.use_goal_factors)
      graph.add(add_goal_distance_factor(t, _params.goal_factor_params));
    if (_params.use_energy_factors)
      graph.add(add_energy_factors(t, _params.goal_factor_params));
  }
