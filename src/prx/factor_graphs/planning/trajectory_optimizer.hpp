...
{
public:
  trajectory_optimizer(const trajectory_t _traj, system_ptr_t _plant, std::shared_ptr<system_group_t> _sys_group,
                       std::string logger = "")
    : traj_original(_traj)
  ...
  // trajectory_optimizer(const trajectory_t _traj, const plan_t , system_ptr_t _plant, std::shared_ptr<system_group_t>
  // _sys_group, std::string logger = "") 	: traj_original(_traj), 	  logger(out_path + "traj_opt_" + logger + ".txt", '
  // ',
  // "-"), 	  traj_internal(_sys_group -> get_state_space()), 	  plan(_sys_group -> get_control_space()),
  // plan_input(_plan)
  // {
  // 	std::cout << "traj_original size: " << traj_original.size() << std::endl;
  // 	sys_group = _sys_group;
