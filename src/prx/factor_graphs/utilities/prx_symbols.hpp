...
public:
   ...
   */
  static prx_symbol_t state_symbol(int64_t t)
  {
    return prx_symbol_t("Xi", 0, t);
  }

  static prx_symbol_t control_symbol(int64_t t)
  {
  ...
  /// Return link id.
  inline uint8_t state_Idx() const
  {
    return state_idx_;
  }
...
private:
   ...
   * @{
   */
  static constexpr size_t kMax_uchar_ = std::numeric_limits<int8_t>::max();
  static constexpr size_t kMax_state_ = std::numeric_limits<int8_t>::max();
  // bit counts
  static constexpr size_t key_bits = sizeof(gtsam::Key) * 8;
  static constexpr size_t ch1_bits = sizeof(int8_t) * 8;
  static constexpr size_t ch2_bits = sizeof(int8_t) * 8;
  static constexpr size_t state_bits = sizeof(int8_t) * 8;
  static constexpr size_t time_bits = key_bits - ch1_bits - ch2_bits - state_bits;
  // masks
  static constexpr gtsam::Key ch1_mask = gtsam::Key(kMax_uchar_) << (key_bits - ch1_bits);
  static constexpr gtsam::Key ch2_mask = gtsam::Key(kMax_uchar_) << (key_bits - ch1_bits - ch2_bits);
  static constexpr gtsam::Key state_mask = gtsam::Key(kMax_state_) << (key_bits - ch1_bits - ch2_bits - state_bits);
  static constexpr gtsam::Key time_mask = ~(ch1_mask | ch2_mask | state_mask);
  /**@}*/
};
...
template <>
struct std::hash<prx::prx_symbol_t>
{
  std::size_t operator()(prx::prx_symbol_t const& s) const noexcept
  {
    std::size_t h1 = std::hash<std::string>{}(std::string(s));
