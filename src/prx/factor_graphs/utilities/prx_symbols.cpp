...
// #include <factor_graphs/utils/DynamicsSymbol.h>
...
/* ************************************************************************* */
prx_symbol_t::prx_symbol_t() : c1_(0), c2_(0), state_idx_(0), t_(0)
{
}

/* ************************************************************************* */
prx_symbol_t::prx_symbol_t(const prx_symbol_t& key) : c1_(key.c1_), c2_(key.c2_), state_idx_(key.state_idx_), t_(key.t_)
{
  // std::cout << "(copy constructor) t: " << t_ << std::endl;
}

/* ************************************************************************* */
prx_symbol_t::prx_symbol_t(const std::string& s, int8_t state_idx, int64_t t) : state_idx_(state_idx), t_(t)
{
  if (s.length() > 2)
  {
  ...
  // printf("%c%c%d%lld\n", c1_, c2_, state_idx_, t_);
  // std::cout << c1_ << c2_ << t_ << std::endl;
}
