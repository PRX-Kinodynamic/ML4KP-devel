...
class potential_energy_factor_t : public gtsam::NoiseModelFactor1<Eigen::VectorXd>
{
public:
  /**
   ...
   */
  potential_energy_factor_t(const gtsam::noiseModel::Base::shared_ptr& cost_model, gtsam::Key xt_key,
                            system_ptr_t _sys_ptr, Eigen::VectorXd& _goal, double _ti, goal_factor_params_t _params)
  ...
  virtual ~potential_energy_factor_t()
  {
  }
  ...
  Eigen::VectorXd compute_error(Eigen::VectorXd xt_v) const
  {
    ...
    error[0] = energy_goal - system_ptr->potential_energy();
                                        ...
                                        boost::optional<Eigen::MatrixXd&> H1 = boost::none) const override
  {
    if (H1)
    {
      std::function<Eigen::VectorXd(Eigen::VectorXd)> fp =
          std::bind(&potential_energy_factor_t::compute_error, this, std::placeholders::_1);
  ...
  {
    std::cout << s << "potential_energy_factor";
    Base::print("", keyFormatter);
  }

private:
  using This = potential_energy_factor_t;
