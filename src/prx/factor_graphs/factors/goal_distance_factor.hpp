...
public:
   ...
   */
  goal_distance_factor_t(const gtsam::noiseModel::Base::shared_ptr& cost_model, gtsam::Key xt_key,
                         system_ptr_t _sys_ptr, int _ti, Eigen::VectorXd _goal, goal_factor_params_t _params)
    : Base(cost_model, xt_key)
  {
    system_ptr = _sys_ptr;
    auto ss = system_ptr->get_state_space();

    prx_assert(_params.T >= _ti,
               "Total time step T (" << _params.T << ") must be greater than or equal than time step (" << _ti << ").")
        prx_assert(0 < _params.theta && _params.theta <= 1, "Theta must be \\in (0,1]");
    prx_assert(_params.error_scale.size() == ss->get_dimension(),
               "Error_scale must be the same size as the state space! ");
