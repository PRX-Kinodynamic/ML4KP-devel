...
public:
   ...
   */
  kinetic_energy_factor_t(const gtsam::noiseModel::Base::shared_ptr& cost_model, gtsam::Key xt_key,
                          system_ptr_t _sys_ptr, Eigen::VectorXd& _goal, int _ti, goal_factor_params_t _params)
                                        ...
                                        boost::optional<Eigen::MatrixXd&> H1 = boost::none) const override
  {
    if (H1)
    {
      std::function<Eigen::VectorXd(Eigen::VectorXd)> fp =
          std::bind(&kinetic_energy_factor_t::compute_error, this, std::placeholders::_1);
