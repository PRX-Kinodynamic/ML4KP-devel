...
class quadratic_cost_factor_t : public gtsam::NoiseModelFactor1<Eigen::Vector<double, DIM_0>>
{
  using Base = gtsam::NoiseModelFactor1<Eigen::Vector<double, DIM_0>>;

  using _Matrix = Eigen::Matrix<double, DIM_0, DIM_0>;
  using _Vector = Eigen::Vector<double, DIM_0>;

public:
  quadratic_cost_factor_t(const gtsam::Key& key, const _Vector& goal, const _Matrix cost, const double sigma = 1e0)
