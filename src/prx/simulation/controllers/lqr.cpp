...
void lqr_t::compute_K(Eigen::MatrixXd& A, Eigen::MatrixXd& B)
{
...
void lqr_t::compute_K()
{
  ...
  plant->get_state_space()->copy_from(goal);
  plant->get_control_space()->copy_from(u_goal);
  plant->linearize(A, B);
  compute_K(A, B);
}
...
void lqr_t::compute_controls()
{
  ...
  U = -K * (X - goal->vector());
  // PRX_DEBUG_VAR_1(X.transpose());
  // PRX_DEBUG_VAR_1(X_goal.transpose());
  // PRX_DEBUG_VAR_1(U.transpose());
  // PRX_DEBUG_VAR_1(K);
