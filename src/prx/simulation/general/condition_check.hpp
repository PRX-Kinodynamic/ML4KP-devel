...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const condition_check_t& obj)
  {
    for (auto type : obj.available_types)
    {
      if (obj.condition_type == type.second)
      {
        os << "[" << type.first << "]: ";
      }
    }
    os << obj.iteration_counter << " / " << obj.condition_check;
