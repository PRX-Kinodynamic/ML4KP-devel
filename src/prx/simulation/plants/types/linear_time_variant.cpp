...
bool ltv_t::linearize_numerical(space_point_t xt, space_point_t ut, double epsilon)
{
  ...
  for (int i = 0; i < ss_dim; i++)
  {
    ...
    get_state_space()->copy_from(x_plus);
    propagate(epsilon);
    // get_state_space() -> copy_to_vector(xd_plus);
    get_derivative_space()->copy_to(xd_plus);

    get_state_space()->copy_from(x_minus);
    propagate(epsilon);
    // get_state_space() -> copy_to_vector(xd_minus);
    get_derivative_space()->copy_to(xd_minus);
  ...
  for (int i = 0; i < cs_dim; i++)
  {
    ...
    get_control_space()->copy_from(u_plus);
    // compute_derivative();
    propagate(epsilon);
    get_derivative_space()->copy_to(xd_plus);

    get_control_space()->copy_from(u_minus);
    propagate(epsilon);
    get_derivative_space()->copy_to(xd_minus);
  ...
  get_state_space()->copy_from(mem_aux);
