...
namespace prx
{
	...
	planar_2link_t::planar_2link_t(const std::string& path) : ltv_t(path)
	{
		...
		_tau_0=_tau_1=0;
		control_memory = {&_tau_0,&_tau_1};
		input_control_space = new space_t("EE",control_memory,"Torque");
		input_control_space->set_bounds({-7,-7},{7,7});
		...
		parameter_space = new space_t("EEEEEEEEE", parameter_memory, "planar_2link_params");
	...
	planar_2link_t::~planar_2link_t()
	{

	}

	void planar_2link_t::propagate(const double simulation_step)
	{
	...
	void planar_2link_t::update_configuration()
	{
	...
	void planar_2link_t::compute_derivative()
	{
        ...
        const double u1 = _tau_0;//    - 1.0 * d1 * theta1dot;
        const double u2 = _tau_1;// - 1.0 * d2 * theta2dot;
	...
	double planar_2link_t::kinetic_energy()
	{
		const double theta2 = _theta2;
      const double theta1 = _theta1 - M_PI / 2.0;
      const double theta1dot = _theta1dot;
	...
	// void planar_2link_t::compute_control()
	// {
		// const double theta2 = _theta2;
  //     const double theta1 = _theta1 - M_PI / 2.0;
  //     const double theta1dot = _theta1dot;
  ...
  //     double m = mass;
	...
	Eigen::MatrixXd planar_2link_t::get_mass_matrix()
	{
		const double theta2 = _theta2;
      const double theta1 = _theta1 - M_PI / 2.0;
      const double theta1dot = _theta1dot;
      ...
      // TODO: Fix this - return only M
      return M * th_dot;
	}

	Eigen::VectorXd planar_2link_t::get_coriolis_vector()
	{
		const double theta2 = _theta2;
      const double theta1 = _theta1 - M_PI / 2.0;
      const double theta1dot = _theta1dot;
	...
	Eigen::VectorXd planar_2link_t::get_gravity_vector()
	{
				const double theta2 = _theta2;
      const double theta1 = _theta1 - M_PI / 2.0;
      const double theta1dot = _theta1dot;
	...
	bool planar_2link_t::linearize(space_point_t xt, space_point_t ut, double epsilon)
   {
