...
void mushr_t::compute_derivative()
{
  ...
  const double gamma{ _current_vel * std::tan(_u[1]) / 0.23 };
  PRX_DEBUG_VAR_2(gamma, _u.transpose());
  ...
  // auto Vb = _Tdot * _T.inverse();
  // auto Vb = _T.inverse() * _Tdot;
  _linear_v = _spatial_twist.head(3);
  // _omega = vee(_Tdot.linear() * _T.linear())[2];
  _omega = (_T.linear() * _body_twist.tail(3))[2];
  PRX_DEBUG_VAR_2(_linear_v.transpose(), _omega);
}
