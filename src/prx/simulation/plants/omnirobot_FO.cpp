...
omnirobot_FO_t::omnirobot_FO_t(const std::string& path) : plant_t(path), D(4, 3), Dp(3, 4)  //, mu(4)
{
  ...
  D << +std::sin(a1), std::cos(a1), 1., -std::sin(a1), std::cos(a1), 1., -std::sin(a1), -std::cos(a1), 1.,
      +std::sin(a1), -std::cos(a1), 1.;

  Dp << 0.3536, -0.3536, -0.3536, 0.3536, 0.3536, 0.3536, -0.3536, -0.3536, 0.2500, 0.2500, 0.2500, 0.2500;
...
bool omnirobot_FO_t::connect_points(space_point_t origin, space_point_t local_goal, plan_t& plan, trajectory_t& traj,
                                    double dist_limit, double tolerance)
{
  traj.clear();
  plan.clear();
  ...
  while (tolerance < dist && d_traj < dist_limit)
  {
    ...
    if (dist > last_dist)
    {
      prx_warn("omnirobot_FO_t::connect_points: Not getting close to local goal!") return false;
    }
...
void omnirobot_FO_t::compute_derivative()
{
  ...
  Eigen::Matrix3d rot;
  rot << std::cos(theta - M_PI / 4), -std::sin(theta - M_PI / 4), 0, std::sin(theta - M_PI / 4),
      std::cos(theta - M_PI / 4), 0, 0, 0, 1;
