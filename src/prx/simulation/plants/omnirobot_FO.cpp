...
namespace prx
{
	...
	omnirobot_FO_t::omnirobot_FO_t(const std::string& path) : plant_t(path), D(4,3), Dp(3,4)//, mu(4)
	{
		...
		geometries["wheel_1"]->initialize_geometry({.034,.013});
		...
		geometries["wheel_2"]->initialize_geometry({.034,.013});
		...
		geometries["wheel_3"]->initialize_geometry({.034,.013});
		geometries["wheel_3"]->generate_collision_geometry();
		geometries["wheel_3"]->set_visualization_color("0xff0000");
		...
		geometries["wheel_4"]->initialize_geometry({.034,.013});
		geometries["wheel_4"]->generate_collision_geometry();
		geometries["wheel_4"]->set_visualization_color("0x0000ff");
	...
	void omnirobot_FO_t::update_configuration()
	{
		...
		w1_tr.translation() = (vector_t(.125,0,0));
		w2_tr.linear() = (Eigen::AngleAxisd(M_PI/2.0, Eigen::Vector3d::UnitX()).toRotationMatrix());
		w2_tr.translation() = (vector_t(0,0.125,0));
		w3_tr.linear() = (Eigen::AngleAxisd(M_PI/2.0, Eigen::Vector3d::UnitY()).toRotationMatrix());
		w3_tr.translation() = (vector_t(-.125,0,0));
		w4_tr.linear() = (Eigen::AngleAxisd(M_PI/2.0, Eigen::Vector3d::UnitX()).toRotationMatrix());
		w4_tr.translation() = (vector_t(0,-0.125,0));
    ...
    // PRX_DEBUG_PRINT
		...
		w1 = U[0] / (1. - (mu[0] * mass * gravity * radius) / (4. * stall_torque));
		w2 = U[1] / (1. - (mu[1] * mass * gravity * radius) / (4. * stall_torque));
		w3 = U[2] / (1. - (mu[2] * mass * gravity * radius) / (4. * stall_torque));
		w4 = U[3] / (1. - (mu[3] * mass * gravity * radius) / (4. * stall_torque));
