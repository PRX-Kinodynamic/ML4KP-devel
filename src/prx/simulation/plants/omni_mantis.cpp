...
omni_mantis_t::omni_mantis_t(const std::string& path) : plant_t(path)
{
  ...
  state_space->set_bounds({ -30, -30, .5, -3.14, 0, -.75, -.5, -.25 }, { 30, 30, 1.7, 3.14, 1.5, .75, .5, .25 });

  input_control_space = new space_t("EEEE", control_memory, "OmniControl");
  input_control_space->set_bounds({ -.5, -.5, -.5, -.1 }, { .5, .5, .5, .1 });
...
void omni_mantis_t::update_configuration()
{
  ...
  if (fabs(_vx * s_theta + _vy * c_theta) > PRX_EPSILON || fabs(_vx * c_theta - _vy * s_theta) > PRX_EPSILON)
  {
  ...
  const auto joint_values = [&, this](double sx, double sy, int start_index) {
    ...
    transforms[start_index]->translation() =
        (vector_t(_x + (rot_sx + rot_ax) / 2, _y + (rot_sy + rot_ay) / 2, (sz + az) / 2));
