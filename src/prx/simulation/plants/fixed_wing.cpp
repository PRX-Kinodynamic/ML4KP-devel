...
fixed_wing_t::fixed_wing_t(const std::string& path) : plant_t(path)
{
  ...
  state_memory = { &_x, &_y, &_z, &_V, &_gamma, &_chi, &_alpha, &_mu, &_thrust };
  ...
  state_space->set_bounds({ -9, -6.5, 1.07, 1, -1.0, -3.14, -.8, -.8, 0.0 }, { 9, 6.5, 15, 8, 1.0, 3.14, .8, .8, 8.0 });
  ...
  input_control_space->set_bounds({ 1, -.8, -.8 }, { 8, .8, .8 });
  ...
  derivative_memory = { &_d_x, &_d_y, &_d_z, &_d_V, &_d_gamma, &_d_chi, &_d_alpha, &_d_mu, &_d_thrust };
...
void fixed_wing_t::compute_derivative()
{
  ...
  //       const double sin_gamma = c_alpha * c_beta * s_theta_p - s_beta * s_phi * c_theta_p - s_alpha * c_beta *
  //       c_theta_p * c_phi;
  //       // gamma = asin(sin_gamma);
  //       const double c_mu_c_gamma = s_alpha * s_theta_p + c_alpha * c_theta_p * c_phi;

  //       const double s_mu_c_gamma = c_alpha * s_beta * s_theta_p + c_beta * s_phi * c_theta_p - s_alpha * s_beta *
  //       c_phi * c_theta_p;

  //       const double c_gamma = cos(asin(sin_gamma));
  //       const double s_chi_c_gamma = c_alpha * c_beta * c_theta_p * s_psi + s_beta * ( s_phi * s_theta_p * s_psi +
  //       c_phi * c_psi ) + 	s_alpha * c_beta * ( c_phi * s_theta_p * s_psi - s_phi * c_psi );
