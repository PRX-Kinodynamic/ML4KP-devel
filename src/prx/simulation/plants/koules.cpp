...
n_vector_t<2> collision_response(n_vector_t<2> x1, n_vector_t<2> x2, n_vector_t<2> v1, n_vector_t<2> v2, double m1,
                                 double m2)
{
...
void koules_t::propagate(const double simulation_step)
{
  ...
  for (int i = 0; i < koules_size; i++)
  {
    ...
    double dist2 =
        (koules_state[i][0] - x) * (koules_state[i][0] - x) + (koules_state[i][1] - y) * (koules_state[i][1] - y);
