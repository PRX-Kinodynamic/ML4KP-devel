...
namespace prx
{
class delivery_bot_first_order_t : public plant_t
{
public:
  delivery_bot_first_order_t(const std::string& path);

  virtual ~delivery_bot_first_order_t();
...
PRX_REGISTER_SYSTEM(delivery_bot_first_order_t, FO_treaded_vehicle)

PRX_REGISTER_VELOCITY_FN(FO_treaded_vehicle, [](prx::system_ptr_t sys_ptr) {
  auto tv_fo = std::dynamic_pointer_cast<prx::delivery_bot_first_order_t>(sys_ptr);
  if (!tv_fo)
