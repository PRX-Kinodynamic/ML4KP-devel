...
pendulum_t::pendulum_t(const std::string& path) : plant_t(path)
{
...
bool pendulum_t::linearize(Eigen::MatrixXd& A, Eigen::MatrixXd& B, Eigen::MatrixXd& C, Eigen::MatrixXd& D,
                           space_point_t xt, space_point_t ut, double epsilon)
{
  if (xt != nullptr && ut != nullptr && xt->at(0) != 0 && xt->at(1) != 0)
    return false;
