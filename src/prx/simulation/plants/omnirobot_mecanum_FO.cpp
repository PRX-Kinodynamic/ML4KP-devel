...
namespace prx
{
  ...
  //   			0.2500,  0.2500,  0.2500,  0.2500;
	...
	bool omnirobot_mecanum_FO_t::connect_points(space_point_t origin, space_point_t local_goal, plan_t& plan, trajectory_t& traj, double dist_limit, double tolerance)
	{
		traj.clear();
    	plan.clear();
		...
		{	
			...
			if ( dist > last_dist)
			{
				prx_warn("omnirobot_mecanum_FO_t::connect_points: Not getting close to local goal!")
				return false;
			}
	...
	void omnirobot_mecanum_FO_t::compute_derivative()
	{
		...
		rot << std::cos(theta), -std::sin(theta), 0,
			   ...
			   0, 0, 1;
		Xd = rot * ( ( R / (4.0 * l_ab ) ) * Dp) * U; // (3x3) * (3x4) * (4x1)
		// Xd = ( ( R / (4.0 * l_ab ) ) * Dp) * U; // (3x3) * (3x4) * (4x1)
		x_d = Xd[0];
		y_d = Xd[1];
		theta_d = Xd[2];
