...
omnirobot_mecanum_FO_t::omnirobot_mecanum_FO_t(const std::string& path) : plant_t(path), D(4, 3), Dp(3, 4)  //, mu(4)
{
  x = y = theta = 0;
  ...
  input_control_space->set_bounds({ -128, -128, -128, -128 }, { 128, 128, 128, 128 });

  geometries["body"] = std::make_shared<geometry_t>(geometry_type_t::BOX);
  geometries["body"]->initialize_geometry({ l_a * 1.8, l_b * 3, .1 });
  ...
  geometries["wheel_1"]->initialize_geometry({ R, .013 });
  ...
  geometries["wheel_2"]->initialize_geometry({ R, .013 });
  ...
  geometries["wheel_3"]->initialize_geometry({ R, .013 });
  ...
  geometries["wheel_4"]->initialize_geometry({ R, .013 });
  ...
  Dp << -l_ab, +l_ab, -l_ab, +l_ab, +l_ab, +l_ab, +l_ab, +l_ab, +1, -1, -1, +1;
  ...
  // FOR USING FRICTION VALUES PER WHEEL
  ...
  // FOR USING THE SAME FRICTION VALUES IN ALL WHEELS
  parameter_memory = { &mu_0 };
  parameter_space = new space_t("E", parameter_memory, "omnirobot_FO_params");
  parameter_space->set_bounds({ 0 }, { 2 });
}

omnirobot_mecanum_FO_t::~omnirobot_mecanum_FO_t()
{
}

void omnirobot_mecanum_FO_t::propagate(const double simulation_step)
{
  integrator->integrate(simulation_step);
}

void omnirobot_mecanum_FO_t::update_configuration()
{
  ...
  w1_tr.linear() = (Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitY()).toRotationMatrix());
  w1_tr.translation() = (vector_t(l_a, l_b, 0));
  w2_tr.linear() = (Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitY()).toRotationMatrix());
  w2_tr.translation() = (vector_t(-l_a, l_b, 0));
  w3_tr.linear() = (Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitY()).toRotationMatrix());
  w3_tr.translation() = (vector_t(-l_a, -l_b, 0));
  w4_tr.linear() = (Eigen::AngleAxisd(M_PI / 2.0, Eigen::Vector3d::UnitY()).toRotationMatrix());
  w4_tr.translation() = (vector_t(l_a, -l_b, 0));
...
bool omnirobot_mecanum_FO_t::connect_points(space_point_t origin, space_point_t local_goal, plan_t& plan,
                                            trajectory_t& traj, double dist_limit, double tolerance)
{
  traj.clear();
  plan.clear();
  ...
  while (tolerance < dist && d_traj < dist_limit)
  {
    ...
    if (dist > last_dist)
    {
      prx_warn("omnirobot_mecanum_FO_t::connect_points: Not getting close to local goal!") return false;
    }
...
void omnirobot_mecanum_FO_t::compute_derivative()
{
  ...
  //   rot * ((R /  4.0 * (3x3) * (3x4) * (4x1)
  Xd = rot * ((R / (4.0 * l_ab)) * Dp) * U;
  x_d = Xd[0];
  y_d = Xd[1];
  theta_d = Xd[2];

  // std::cout << "[Omni] " << mu_0 << " " << mu_1 << " " << mu_2 << " " << mu_3 << " ";
