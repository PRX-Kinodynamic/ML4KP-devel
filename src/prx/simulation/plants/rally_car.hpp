...
protected:
  ...
  const double C = 1.6;
  const double D = .52;
  // auto rally_car_vel_fn = [](prx::system_ptr_t s) {
public:
  static double rally_car_vel_fn(prx::system_ptr_t s)
  {
    return std::sqrt(std::pow(s->get_state_space()->get_bounds()[3].second, 2) +
                     std::pow(s->get_state_space()->get_bounds()[4].second, 2));
  };
};
}  // namespace prx
...
PRX_REGISTER_VELOCITY_FN(rally_car, prx::rally_car_t::rally_car_vel_fn)