...
public:
  ...
  bool connect_points(space_point_t origin, space_point_t local_goal, plan_t& plan, trajectory_t& traj,
                      double dist_limit = std::numeric_limits<double>::infinity(), double tolerance = 0.1);
...
auto omnirobot_mecanum_FO_or_vel_fn = [](prx::system_ptr_t sys_ptr) {
  double vel = 0;
  return vel;
};
PRX_REGISTER_VELOCITY_FN(omnirobot_mecanum_FO_t, omnirobot_mecanum_FO_or_vel_fn)
