...
trajectory_t& trajectory_t::operator=(const trajectory_t& t)
{
  prx_assert(state_space->get_space_name() == t.state_space->get_space_name(),
             "Trajectory buffers have mismatched state spaces.");
...
{
  prx_assert(state_space->get_space_name() == t.state_space->get_space_name(),
             "Appending trajectory from different space.");
...
void trajectory_t::copy_onto_back(const space_t* space)
{
  prx_assert(state_space->get_space_name() == space->get_space_name(),
             "Trying to add a new point from the wrong space (" + space->get_space_name() +
                 " in a trajectory with space" + state_space->get_space_name() + ")");
...
std::string trajectory_t::print(unsigned precision) const
{
  ...
  for (const space_point_t& st : *this)
  {
    ...
    out << state_space->print_point(st, precision) << std::endl;
    counter++;
  }
  return out.str();
}
