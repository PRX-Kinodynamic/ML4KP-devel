...
void collision_checker_t::add_collision_group(const std::string& group_name, const std::vector<system_ptr_t>& in_plants,
...
{
  prx_assert(collision_groups.count(group_name) != 0,
             "Trying to get a collision group with name " << group_name << " that hasn't been built");
