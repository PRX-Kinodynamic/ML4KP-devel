...
void dopri5_t::step()
{
  ...
  _compute_derivative();
  _derivative_space->copy_to(k2);

  // Compute k3 = f(X0 + C3h, Y0 + h * (a31 * k1 + a32 * k2))
  _derivative_space->copy_to(k_cache);
  k_cache->multiply(_h * a32);
  k_cache->add_multiply(_h * a31, k1);
  _state_space->copy(yn, start_integration_state);
  yn->add(k_cache);
  _state_space->copy_from(yn);
  _compute_derivative();
  _derivative_space->copy_to(k3);
  ...
  for (int i = 0; i < n; ++i)
  {
    yn->at(i) = start_integration_state->at(i) +  // no-lint
                _h * (b1 * k1->at(i) + b2 * k2->at(i) + b3 * k3->at(i) + b4 * k4->at(i) + b5 * k5->at(i) +
                      b6 * k6->at(i) + b7 * k7->at(i));
  }
