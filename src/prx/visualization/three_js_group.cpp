                                   ...
                                   const std::vector<std::shared_ptr<movable_object_t>>& in_obstacles)
{
  ...
  for (auto&& sys : in_plants)
  {
    ...
    if (plant)
    {
      ...
      {
        prx_assert(geoms[i].first == configs[i].first,
                   "Geometry and configuration lists don't match in " << plant->get_object_name());
        auto g = geoms[i].second;
        auto config = configs[i].second;
        auto g_ptr = g.lock();
        auto info = std::make_shared<vis_info_t>();
...
void three_js_group_t::add_vis_infos(info_geometry_t info_type, std::vector<trajectory_t>& tree_vis,
                                     std::string body_name, space_t* state_space, std::string color)
{
...
void three_js_group_t::add_vis_infos(info_geometry_t info_type, const trajectory_t& traj, std::string body_name,
                                     space_t* state_space, std::string color)
{
  ...
  prx_assert(selected_info, "Tried to generate extra info geometries with rigid body name "
                                << body_name << ", but that geometry doesn't exist.");
  if (traj.size() == 0)
  {
...
void three_js_group_t::add_vis_infos(info_geometry_t info_type, std::vector<vector_t> positions, std::string color,
                                     double radius)
{
  info_geoms.push_back({ info_type, std::move(positions), color, radius });
}

void three_js_group_t::add_detailed_vis_infos(info_geometry_t info_type, const trajectory_t& traj,
                                              std::string body_name, const space_t* state_space, std::string color)
{
  std::shared_ptr<vis_info_t> selected_info;
  for (auto&& element : state_infos)
  {
...
void three_js_group_t::add_animation(const trajectory_t& traj, const space_t* state_space, space_point_t default_state)
{
  double timestamp = 0;
...
void three_js_group_t::output_graph_to_csv(std::string filename, std::string sep)
{
  ...
  for (auto&& element : info_geoms)
  {
    ...
    if ((element.first == info_geometry_t::LINE || element.first == info_geometry_t::FULL_LINE) &&
        element.second.size() >= 2)
    {
      ...
      {
        csv_string += std::to_string(element.second[i].x()) + sep + std::to_string(element.second[i].y()) + sep +
                      std::to_string(element.second[i].z()) + sep + std::to_string(color_vals.x()) + sep +
                      std::to_string(color_vals.y()) + sep + std::to_string(color_vals.z()) + "\n";
      }
    }
...
void three_js_group_t::output_html(std::string filename)
{
  const auto vis_geometry_init = [](std::shared_ptr<vis_info_t>& element, std::string& js_string) {
    if (element->geom_type == geometry_type_t::BOX)
    {
      js_string += "var geometry = new THREE.BoxGeometry( " + std::to_string(element->geom_params[0]) + ", " +
                   std::to_string(element->geom_params[1]) + ", " + std::to_string(element->geom_params[2]) + " );";
    }
    else if (element->geom_type == geometry_type_t::CYLINDER)
    {
      js_string += "var geometry = new THREE.CylinderGeometry( " + std::to_string(element->geom_params[0]) + ", " +
                   std::to_string(element->geom_params[0]) + ", " + std::to_string(element->geom_params[1]) + ", 32 );";
    ...
    {
      js_string +=
          "var geometry = new THREE.SphereGeometry( " + std::to_string(element->geom_params[0]) + ", 32, 32 );";
    }
  ...
  for (auto&& element : obstacle_infos)
  {
    ...
    js_string += "mesh.setRotationFromQuaternion(new THREE.Quaternion(" + std::to_string(element->rotation.x()) + "," +
                 std::to_string(element->rotation.y()) + "," + std::to_string(element->rotation.z()) + "," +
  ...
  for (auto&& element : info_geoms)
  {
    ...
    if (element.first == info_geometry_t::LINE && element.second.size() >= 2)
    {
      ...
      for (int i = 0; i < element.second.size(); i++)
      {
        js_string += "positions.push( " + std::to_string(element.second[i].x()) + ", " +
                     std::to_string(element.second[i].y()) + ", " + std::to_string(element.second[i].z()) + " );";
    ...
    else if (element.first == info_geometry_t::FULL_LINE && element.second.size() >= 2)
    {
      ...
      {
        js_string += "positions.push( " + std::to_string(element.second[i].x()) + ", " +
                     std::to_string(element.second[i].y()) + ", " + std::to_string(element.second[i].z() + 0.05) +
                     " );";
        js_string += "colors.push(" + std::to_string(color_vals.x()) + "," + std::to_string(color_vals.y()) + "," +
                     std::to_string(color_vals.z()) + ");";
      }
    ...
    else if (element.first == info_geometry_t::QUAD && element.second.size() >= 4)
    {
      ...
      js_string += "var material = new THREE.MeshPhongMaterial( { color: " + element.third +
                   " , side: THREE.DoubleSide, flatShading: true } );";

      for (int i = 0; i < element.second.size(); i++)
      {
        js_string += "geometry.vertices.push( new THREE.Vector3(" + std::to_string(element.second[i].x()) + ", " +
                     std::to_string(element.second[i].y()) + ", " + std::to_string(element.second[i].z()) + ") );";
      }
  ...
  if (tree_log_file.size() > 0)
  {
    ...
    for (auto t : trajs)
    {
      ...
      for (int i = 0; i < coords.size(); i++)
      {
        ...
        js_string += "positions.push( " + std::to_string(coords[i][0]) + ", " + std::to_string(coords[i][1]) + ", " +
                     std::to_string(coords[i][2] + 0.05) + " );";
    ...
    for (auto pt : grid_pts)
    {
      ...
      // for(int i=0;i<2;i++)
      // {
      js_string += "positions.push( " + std::to_string(x - 1) + ", " + std::to_string(y) + ", " +
    ...
    // 			{
    // 				js_string+="positions.push( "+std::to_string()+", "+std::to_string(element.second[i].y())+",
