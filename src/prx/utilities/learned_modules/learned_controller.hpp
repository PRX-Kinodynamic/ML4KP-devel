...
public:
    ...
    if (normalize_input)
    {
      for (int i = 0; i < states.size(); i++)
      {
        normalized_states.push_back(
            extract_state(normalize_vector(states[i], state_lower_bounds, state_upper_bounds), state_indices));
        normalized_goals.push_back(
            extract_state(normalize_vector(goals[i], state_lower_bounds, state_upper_bounds), goal_indices));
      }
    }
    else
    {
  ...
  void fulfill_query(planner_query_t& query, std::shared_ptr<system_group_t> sg)
  {
    ...
    while (time_so_far < max_duration && !query.goal_check(current))
    {
      ...
      sg->get_control_space()->copy_point_from_vector(query.solution_plan.back().control,
                                                      get_control(state_vec, goal_vec));
      if (debug_controller)
        std::cout << sg->get_control_space()->print_point(query.solution_plan.back().control, 4) << " "
                  << query.solution_plan.back().duration << std::endl;
      sg->propagate(current, query.solution_plan, step_traj);
      if (debug_controller)
        std::cout << sg->get_state_space()->print_point(step_traj.back(), 4) << std::endl;
  ...
  void fulfill_query(planner_query_t& query, rrt_specification_t& spec)
  {
    ...
    while (time_so_far < max_duration && !query.goal_check(current))
    {
      ...
      if (debug_controller)
        std::cout << spec.control_space->print_point(query.solution_plan.back().control, 4) << " "
                  << query.solution_plan.back().duration << std::endl;
      spec.propagate(current, step_plan, step_traj);
      if (debug_controller)
        std::cout << spec.state_space->print_point(step_traj.back(), 4) << std::endl;
