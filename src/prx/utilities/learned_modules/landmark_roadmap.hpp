...
class landmark_roadmap_t
{
    ...
    bool is_node_on_path(node_index_t index, node_index_t path_start)
    {
        ...
        if (index == path_start) return true;
    ...
    int get_next_local_goal(space_point_t s,rrt_query_t& query, rrt_specification_t& spec, learned_controller_t controller,int vertex_id)
    {
        ...
        do 
        {
            ...
            if (spec.valid_check(query.solution_traj) && query.solution_traj.size() > 0)
            {
                // std::cout << "Reached node " << v -> get_index() << " with cost " << (query.solution_traj.size()-1)*simulation_step << std::endl;
                best_idx = v -> get_index();
            }
            else
            {
                // std::cout << "Failed to reach node " << v -> get_index() << std::endl;
                break;
            }
            if (v -> get_successor() == v -> get_index())
            {
                // std::cout << "Reached a dead end" << std::endl;
                break;
            }
            v = vertices[v -> get_successor()];
        } while (true);
        ...
        do 
        {
            ...
            if (spec.valid_check(query.solution_traj) && query.solution_traj.size() > 0)
            {
                // std::cout << "Reached node " << next_index << std::endl;
                best_idx = next_index;
                v = next_v;
            }
            else
            {
                // std::cout << "Failed to reach node " << next_index << std::endl;
                v = next_v;
                break;
            }
