...
public:
   ...
   */
  double &at(unsigned index) { return memory[index]; }
   ...
   * @return Value of the space snapshot at the index.
   */
  const double at(unsigned index) const { return memory[index]; }

  space_snapshot_t(const space_t *const in_parent, unsigned dim)
      : parent(in_parent) {
    memory.resize(dim);
  }
   ...
   */
  inline const int get_dim() { return memory.size(); }

  void add(const space_point_t &obj) {
    prx_assert(obj->get_dim() == memory.size(),
               "Points have different dimension size!: "
                   << obj->get_dim() << " and " << memory.size());
...
typedef std::function<double(const space_point_t &, const space_point_t &)>
    distance_function_t;
...
public:
   ...
   */
  void point_union(const space_point_t &p1, const space_point_t &p2,
                   const space_point_t &pu);
   ...
   */
  void split_point(const space_point_t &ps_to_split, const space_point_t &ps1,
                   const space_point_t &ps2);
   ...
   *
   */
  // void point_addition(const space_point_t& pt1, const space_point_t& pt2,
  // const space_point_t& pt_res);
  ...
  void copy_point(const space_point_t &destination,
                  const space_point_t &source) const;
  void copy_to_vector(std::vector<double> &destination) const;
  void copy_from_vector(const std::vector<double> &source);

  void copy_point_from_vector(const space_point_t &destination,
                              const std::vector<double> &source) const;
  void copy_point_from_vector(const space_point_t &destination,
                              const Eigen::VectorXd &source) const;
  void copy_vector_from_point(std::vector<double> &destination,
                              const space_point_t &source) const;
  void copy_vector_from_point(Eigen::VectorXd &destination,
                              const space_point_t &source) const;

  void copy_point_from_string(const space_point_t &destination,
                              const std::string source, char sep = ' ') const;
  ...
  inline std::string get_space_name() const { return space_name; }

  inline double &at(unsigned index) const {
    prx_assert(index < dimension, "Trying index into space at index "
                                      << index << " with dimension "
                                      << dimension);
    return *addresses[index];
  }

  inline double &operator[](unsigned index) const { return at(index); }

  inline double get_lower_bound(unsigned i) const {
    prx_assert(i < dimension, "Error: Trying to get bound for "
                                  << i
                                  << " that is higher than state dimension "
                                  << dimension << ".");
  ...
  void integrate(const space_point_t &point, const space_t *derivative,
                 double delta_t);
  void integrate(const space_t *derivative, double delta_t);

  void interpolate(const space_point_t &point1, const space_point_t &point2,
                   double t, space_point_t &result) const;

  std::string print_point(const space_point_t &point, unsigned prec = 25) const;

  std::string print_memory(unsigned prec = 25) const;

  // void linearize(const Eigen::Ref<Eigen::VectorXd>, const
  // Eigen::Ref<Eigen::MatrixXd>, double epsilon = 1e-3);
  ...
  static double l1_norm(const space_point_t &p1, const space_point_t &p2) {
    // int i = 0;
    auto fn = [](double accum, std::tuple<space_snapshot_t::iterator,
                                          space_snapshot_t::iterator> &e) {
      double e1, e2;
  ...
  static double lp_norm(const space_point_t &p1, const space_point_t &p2,
                        const double p) {
    // int i = 0;
    // auto fn = [&p](double accum, std::tuple<space_snapshot_t::iterator,
    // space_snapshot_t::iterator>& e)
    // {
    // 	double e1, e2;
    // 	std::tie(e1, e2) = unzip(e);
    ...
    // return std::pow(std::accumulate(zipped.begin(), zipped.end(), 0.0,
    // fn), 1.0/p);
   ...
   */
  static double euclidean_2d(const space_point_t &p1, const space_point_t &p2,
                             int start = 0, int end = 2) {
    // int i = start;
    // static std::function<double(double accum,
    // std::tuple<space_snapshot_t::iterator, space_snapshot_t::iterator>&)> fn
    // = [](double accum, std::tuple<space_snapshot_t::iterator,
    // space_snapshot_t::iterator>& e)
    // {
    // 	double e1, e2;
    ...
    // return std::sqrt(std::accumulate(zipped.begin() + start, zipped.begin() +
    // end, 0.0, fn));
  ...
  topology_t topology_at(unsigned i) { return topology[i]; };
   ...
   */
  static void subtract(const space_point_t result, const space_point_t p1,
                       const space_point_t p2) {
    prx_assert(
        result->parent->space_name == p1->parent->space_name,
        "To substract two points, the spaces must be the same. Result is "
            << result->parent->space_name << " but p1 is "
            << p1->parent->space_name);
    prx_assert(p1->parent->space_name == p2->parent->space_name,
               "To substract two points, the spaces must be the same. p1 is "
                   << p1->parent->space_name << " but p2 is "
