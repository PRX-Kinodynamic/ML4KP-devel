...
{
  // grid_length = 22.2 / c_size + 22; // added extra space for local map collection - 21 extra cells in each direction
  // for 41x41 local map grid_width = 22.2 / c_size + 22;
  grid_length = 22.2 / c_size + 51;  // added extra space for local map collection - 21 extra cells in each direction
                                     // for 41x41 local map
                                  ...
                                  const double _goaly, double c_size)
{
  ...
  // dl = (origin[0] - (delta_x+grid_length / 2.0)) * cell_size + grid_length/2.0 - map_l/2.0*cell_size;		//
  // (X_local_bleft-X_global_bleft)*cell_size dw = (origin[1] - (delta_y+grid_width / 2.0)) * cell_size + grid_width/2.0
  // - map_w/2.0*cell_size;
  else
  {
  ...
  //                 max_val = max_val > get_score(x,y) || get_score(x,y) == std::numeric_limits<double>::max() ?
  //                 max_val : get_score(x,y);
  //                 // max_val = max_val > global_cost_map[x][y] || global_cost_map[x][y] ==
  //                 std::numeric_limits<double>::max() ? max_val : global_cost_map[x][y];
  //         }
  //         // std::cout << std::endl;
  // }
  // std::cout << max_val << std::endl;
  // std::ofstream outfile;
  // std::string map_string = "P2\n"+std::to_string(grid_length)+"
...
double** heuristic_map_t::init_rand_map(double _start[2], int dim[2], double dx, double dy, double p_density)
{
  ...
  // // Free goal area
  // for(int i = std::max(0, (int) (local_goal[0] - 1.0/cell_size)); i < std::min(dim[0], (int) (local_goal[0]
  // + 1.0/cell_size)); i++)
  // {
  // 	for(int j = std::max(0, (int) (local_goal[1] - 1.0/cell_size)); j < std::min(dim[1], (int) (local_goal[1]
  // + 1.0/cell_size)); j++)
  // 	{
  // 		int local[2] = {i, j};
  // 		int global_map[2];
    ...
    */
    while (!obs_fringe.empty())
    {
      ...
      for (node_t n : neighbors)
      {
        ...
        {
          //
          // double g_cur = 0.01*cell_size/1.0*exp(4.6*global_cost_map[n.x][n.y]) + global_h_map[v_cur.x][v_cur.y] +
          // get_num_obs_neighbors(v_cur.x, v_cur.y);
          double g_cur = 1 + global_h_map[v_cur.x][v_cur.y] + get_num_obs_neighbors(v_cur.x, v_cur.y);
          prx_assert((global_h_map[v_cur.x][v_cur.y] != std::numeric_limits<double>::max()), "ugh");
          if (g_cur >= global_h_map[n.x][n.y])
            continue;
