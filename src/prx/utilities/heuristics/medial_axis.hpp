 ...
 */
// static std::function<bool(std::complex<double>,std::complex<double>)> cmpx_comp = [] (std::complex<double> a,
...
public:
  static constexpr std::complex<double> z_obstacle =
...
private:
  ...
  graph_nearest_neighbors_t<space_point_t>* metric_close;
  graph_nearest_neighbors_t<space_point_t>* metric_obstacles;
  ...
  inline bool is_obstacle(int i, int j)
  {
    ...
    return (i < 0 || i >= rows()) || (j < 0 || j >= cols()) || ma_map[i][j].is_obstacle();
  }
...
public:
  ...
  void init()
  {
    ...
    metric_close = new graph_nearest_neighbors_t<space_point_t>(df_ma);
    metric_obstacles = new graph_nearest_neighbors_t<space_point_t>(df_ma);
    graph.clear();
  ...
  void set_sknw(const std::string nodes_file, const std::string sknw_file,
  ...
  std::vector<std::complex<double>> blossom(int i, int j, int n,
   ...
   */
  std::complex<double> integrated_vector_at(int i, int j, double max_magnitud = std::numeric_limits<double>::max(),
                                            double min_magnitud = 0.0, bool avg_if_obstacle = false);
  ...
  void set_map(int x_grid, int y_grid, valid_state_t& valid_state, mapping_f f, space_t* state_space,
