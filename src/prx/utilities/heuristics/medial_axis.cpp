...
namespace prx {
constexpr std::complex<double> ma_cell::
    z_obstacle; // =
                // std::complex<double>(std::numeric_limits<double>::max(),std::numeric_limits<double>::max());

void medial_axis_t::set_map(int x_grid, int y_grid, valid_state_t &valid_state,
                            mapping_f f, space_t *state_space,
...
void medial_axis_t::add_obstacles_to_graph() {
  ...
  std::function<bool(int, int)> surrounded_by_obstacles = [&](int i, int j) {
    ...
    auto vec = {std::make_pair(i - 1, j - 1), std::make_pair(i - 1, j),
                std::make_pair(i - 1, j + 1), std::make_pair(i, j - 1),
                std::make_pair(i, j + 1),     std::make_pair(i + 1, j - 1),
                std::make_pair(i + 1, j),     std::make_pair(i + 1, j + 1)};
    for (auto p : vec) {
      if (p.first < 0 || p.second < 0 || p.first >= rows() ||
          p.second >= cols())
        continue;
      if (!(is_obstacle(p.first, p.second)))
        return false;
    }
    return true;
  };
  ...
  for (int i = 0; i < rows(); ++i) {
    for (int j = 0; j < cols(); ++j) {
      // If this pt is sorrounded by obstacles, no need to add it to the gnn of
      // obstacles
      if (is_obstacle(i, j) && !surrounded_by_obstacles(i, j)) {
        ...
        pt.push_back(j);
        auto node_index =
            obstacles_graph.add_vertex<undirected_node_t, undirected_edge_t>();
        auto node =
            obstacles_graph.get_vertex_as<undirected_node_t>(node_index);
...
void medial_axis_t::set_sknw(const std::string nodes_file,
                             const std::string edges_file,
                             const std::string sknw_file) {
  ...
  for (std::string line; std::getline(ifs_e, line);) {
    ...
    prx_assert(vstrings.size() >= 3,
               "Not enough indexes on a line of file: " << edges_file);

    unsigned long e_index = std::stoul(vstrings[0]);
    edges[e_index].push_back(
        std::complex<double>(std::stod(vstrings[1]), std::stod(vstrings[2])));
  }
  ...
  for (std::string line; std::getline(ifs_n, line);) {
    ...
    std::vector<std::string> vstrings(begin, end);
    unsigned long index;
    prx_assert(vstrings.size() >= 2,
               "Not enough indexes on a line of file: " << nodes_file);
  ...
  for (std::string line; std::getline(ifs_s, line);) {
    ...
    std::vector<std::string> vstrings(begin, end);
    prx_assert(vstrings.size() >= 2,
               "Not enough indexes on a line of file: " << sknw_file);
    ...
    std::function<void(node_index_t, node_index_t, int, int)> add_new_node =
        [&](node_index_t n1, node_index_t n2, int edge1_index,
            int edge2_index) {
          ...
          if (edge1_index + 1 == edge2_index || edge1_index == edge2_index ||
              has_direct_line_of_sight(node_1->point, node_2->point,
                                       los_clearance)) {
            ...
            for (int i = edge1_index; i < edge2_index; ++i) {
              nodes_edges[edge_index] =
                  std::make_pair(node_1->get_index(), node_2->get_index());
              set_as_edge(edge_pts[i].real(), edge_pts[i].imag(), edge_index);
              set_as_edge(edge_pts[i].real(), edge_pts[i].imag() + 1,
                          edge_index);
              set_as_edge(edge_pts[i].real(), edge_pts[i].imag() - 1,
                          edge_index);
              set_as_edge(edge_pts[i].real() + 1, edge_pts[i].imag(),
                          edge_index);
              set_as_edge(edge_pts[i].real() - 1, edge_pts[i].imag(),
                          edge_index);
            }
            ...
            // prx_assert( (candidate/2) != (candidate + (edge_pts.size() -
            // candidate) / 2), "Can't connect nodes!");
            auto node_index =
                graph.add_vertex<undirected_node_t, undirected_edge_t>();
            auto node = graph.get_vertex_as<undirected_node_t>(node_index);
            node->point = state_space->make_point();

            std::vector<double> v = {edge_pts[middle].real(),
                                     edge_pts[middle].imag()};
            // unset_edge(edge_pts[middle].real(), edge_pts[middle].imag());
            set_as_node(edge_pts[middle].real(), edge_pts[middle].imag(),
                        node_index);
            next_node_id++;
...
void medial_axis_t::compute_sknw_and_save_to_files(std::string nodes_file,
                                                   std::string edges_file,
                                                   std::string sknw_file,
                                                   std::string py_sknw,
                                                   std::string py_cmd) {
  prx_assert(goal != nullptr, "Goal not set!");

  std::string cmd = py_cmd + " " + py_sknw + " " + map_file_name + " " +
                    nodes_file + " " + edges_file + " " + sknw_file;
...
void medial_axis_t::add_goal_to_graph() {
  auto create_edges = [&](space_point_t p1, space_point_t p2,
                          node_index_t other, edge_index_t edge_i) {
  ...
  for (auto n : vec) {
    ...
    if (has_direct_line_of_sight(goal, node->point, los_clearance)) {
      ...
      set_as_node(std::floor(node->point->at(0)),
                  std::floor(node->point->at(1)), node->get_index());
...
void medial_axis_t::prepare_graph() {
  prx_assert(stage >= sknw_set,
             "To prepare graph, sknw has to be set/computed.");
...
void medial_axis_t::find_medial_axis() {
  ...
  // {return (std::hash<double>{}(a.real()) ^ (std::hash<double>{}(a.imag()) <<
  // 1) ) < 	(std::hash<double>{}(b.real()) ^ (std::hash<double>{}(b.imag()) <<
  // 1) );};
  ...
  std::function<bool(int, int)> is_ma = [&](int i, int j) {
    ...
    // if (0 <= i-1 && 0 <= j-1 && i-1 < rows() && j-1 < cols() &&
    // !is_obstacle(i-1, j-1)) vs.push_back(-map_close(i-1,j-1));
    if (0 <= i - 1 && 0 <= j + 0 && i - 1 < rows() && j + 0 < cols() &&
        !is_obstacle(i - 1, j + 0))
      vs.push_back(-map_close(i - 1, j + 0));
    // if (0 <= i-1 && 0 <= j+1 && i-1 < rows() && j+1 < cols() &&
    // !is_obstacle(i-1, j+1)) vs.push_back(-map_close(i-1,j+1));
    if (0 <= i + 0 && 0 <= j - 1 && i + 0 < rows() && j - 1 < cols() &&
        !is_obstacle(i + 0, j - 1))
      vs.push_back(-map_close(i + 0, j - 1));
    // if (0 <= i+0 && 0 <= j+0 && i+0 < rows() && j+0 < cols() &&
    // !is_obstacle(i+0, j+0)) vs.push_back(-map_close(i+0,j+0));
    if (0 <= i + 0 && 0 <= j + 1 && i + 0 < rows() && j + 1 < cols() &&
        !is_obstacle(i + 0, j + 1))
      vs.push_back(-map_close(i + 0, j + 1));
    // if (0 <= i+1 && 0 <= j-1 && i+1 < rows() && j-1 < cols() &&
    ...
    for (int k = 0; k < vs.size(); ++k) {
      ...
      // MA_DEBUG(i, j, 181, 350, "c: " << c << " vs[k]: " << vs[k] << " angle:
      // " << ang << " count: " << ma_map[i][j].close_count);
      if (c == -vs[k])
        return true; // std::abs(std::arg(vs[i] + vs[j]));
      ...
      // // 	if (vs[k] == -vs[l] ) return true; // std::abs(std::arg(vs[i] +
      // vs[j]));
      // // 	else if (std::abs(std::acos(dot(vs[k], vs[l]) / (std::abs(vs[k])
      // * std::abs(vs[l])))) >= M_PI / 2.0 ) return true; //
      // std::abs(std::arg(vs[i] + vs[j]));
      // }
    }
  ...
  std::function<double(double, double)> angle_diff = [&](double a1, double a2) {
    ...
    // return std::min(std::fabs(a1 - a2), std::fabs(norm_angle_pi(a1+M_PI,
    // -M_PI, M_PI) - a2 ));
  };
  ...
  std::function<void(ma_pt, double)> set_MA = [&](ma_pt pt, double dist_prev) {
    ...
    // // MA_DEBUG(i,j, 10, 10, "curr_angle: " << curr_angle << " dist_prev: "
    // << dist_prev << " distance: " << angle_diff(curr_angle, dist_prev));
    // // MA_DEBUG(pt.real(), pt.imag(), 10, 10, "sin_angle: " << sin_angle << "
    // dist_prev: " << dist_prev); set_as_edge(EXPAND_CMP(pt));

    // auto close = close_vector_at(EXPAND_CMP(pt));
    // auto close_ang = std::arg(close);
    // std::cout << "\tpt: " << pt << " curr_angle: " << curr_angle << "
    // close_ang: " << close_ang << " diff: " << angle_diff(curr_angle,
    // close_ang) << std::endl; auto cv = pt + close ;//
    // close_vector_at(EXPAND_CMP(pt)); std::cout <<"\tclose: " <<
    // close_vector_at(EXPAND_CMP(pt)) << " cv: " << cv << std::endl;
    ...
    for (auto d : dir4) {
      auto adj = pt + d.first;
      // std::cout << "pt: " << pt << " adj: " << adj << " obst: " << first_obst
      // << std::endl;
      if (is_obstacle(EXPAND_CMP(adj)))
        continue;
      if (!in_bounds(EXPAND_CMP(adj)))
        continue;
      auto close = -close_vector_at(EXPAND_CMP(adj), false);
      auto v = adj + close;
      // std::cout << "\tclose: " << close << " v: " << v << " diff: " <<
      // std::fabs(std::abs(first_obst) - std::abs(v)) << std::endl; if (first)
      // {
      // 	first = false;
      // 	first_obst = v;
      // }
  ...
  // const std::string file_to_ma = lib_path + "/out/ma_map_" +
  // "simple_obstacle" + "_vf_to_ma.txt"; std::ofstream ofs_map;
  ...
  // std::cout << "total metric numbers: " << metric_close -> get_nr_nodes() <<
  // std::endl;
...
void medial_axis_t::compute_close_vector(int i, int j) {
  ...
  std::vector<proximity_node_t *> vec_nodes = metric_close->multi_query(
      pt, 30); // radius_and_closest_query(pt, std::abs(map_close(i,j)));
...
void medial_axis_t::compute_far_vector(int i, int j) {
  ...
  if (is_obstacle(i, j) || !in_bounds(i, j))
    return;
  ...
  } else // Find the medial axis node with direct line of sight from current pos
         // that is closest to the goal
  {
    ...
    std::vector<proximity_node_t *> vec_close =
    ...
    std::function<void(undirected_node_t *)> insert_nodes =
        [&](undirected_node_t *ni) {
          ...
          if (has_direct_line_of_sight(pt, ni->point, los_clearance)) {
            ...
            auto next_node =
                graph.get_vertex_as<undirected_node_t>(ni->get_best_neighbor());
    ...
    for (auto n : vec_nodes) {
      ...
      // MA_DEBUG(i, j, 389, 77, "n: " << node -> point -> at(0) << " " << node
      // -> point -> at(1) <<
      //  " cost: " << node -> get_cost_to_go() << " best_cost: " << best_cost
    ...
    // If a sln has not been found yet, just point to the closer without
    // checking line of sight
    if (best_cost == std::numeric_limits<double>::infinity()) {
      auto node_ptr =
          static_cast<undirected_node_t *>(metric_close->single_query(pt));
      // v2(0) = node -> point -> at(0);
      // v2(1) = node -> point -> at(1);
...
void medial_axis_t::compute_close_vector_field() {
  prx_assert(
      stage >= map_set,
      "To compute close VF, graph has to be ready (call prepare_graph).");
  add_obstacles_to_graph();

  std::deque<std::pair<ma_pt, ma_pt>> cells;
  ma_pt a;
  double dist_aux = 0.0;
  std::function<void(ma_pt, ma_pt)> add_to_cells = [&](ma_pt pt,
                                                       ma_pt obstacle) {
    if (in_bounds(EXPAND_CMP(pt)) && !is_obstacle(EXPAND_CMP(pt))) {
    ...
    for (auto d : dir4) {
      ...
      // std::cout << b << " " << c << " a: " << a << " pt: " << pt <<
      // std::endl; auto e = b && c ? dist_to_obstacle(a) : 0.0;
      // PRX_DEBUG_PRINT;
      // auto f = b && c ? dist_to_obstacle(pt) + d.second : 0.0;
      // std::cout << b << " " << c << " " << e << " " << f << std::endl;
      if (in_bounds(EXPAND_CMP(a)) && !is_obstacle(a.real(), a.imag()) &&
          dist_to_obstacle(a) > (dist_aux + d.second)) {
        // set_node_as(a.real(), a.imag(), true);
        // if (dist_aux != 0.0)
        // 	set_node_as(pt.real(), pt.imag(), false);
        // std::cout << "pt: " << pt << " pt_dist: " << dist_to_obstacle(pt) <<
        // "cell: " << a << " dist: " << dist_to_obstacle(a) << std::endl;
        // ma_map[a.real()][a.imag()].close_count++;

        cells.push_back(
            std::make_pair(std::complex<double>(EXPAND_CMP(a)), obstacle));
        ...
        if (is_obstacle(EXPAND_CMP(pt))) {
          ...
          // set_node_as(a.real(), a.imag(), true);
          // set_node_as(pt.real(), pt.imag(), false);
          // MA_DEBUG(a.real(), a.imag(), 460, 233, "a: " << a << " pt: " << pt
          // << map_close(a.real(), a.imag())); MA_DEBUG(pt.real(), pt.imag(),
          // 460, 233, "a: " << a << " pt: " << pt << map_close(pt.real(),
          // pt.imag())); MA_DEBUG(a.real(), a.imag(), 461, 233, "a: " << a << "
          // pt: " << pt << map_close(a.real(), a.imag())); MA_DEBUG(pt.real(),
          // pt.imag(), 461, 233, "a: " << a << " pt: " << pt <<
          // map_close(pt.real(), pt.imag()));
        }
        dist_to_obstacle(a) = dist_aux + d.second;
      }
    }
...
std::complex<double> medial_axis_t::close_vector_at(int i, int j,
                                                    bool normalized) {
  prx_assert(stage >= close_ready, "Close VF has not been computed!");
  double u, v;
  if (!in_bounds(i, j))
...
std::complex<double> medial_axis_t::far_vector_at(int i, int j,
                                                  bool normalized) {
...
std::vector<std::complex<double>> medial_axis_t::blossom(int i, int j, int n,
                                                         double max_magnitud,
                                                         double min_magnitud,
                                                         bool avg_if_obstacle) {
  std::vector<ma_pt> r;
  std::map<node_index_t, ma_pt> vectors;
  std::map<node_index_t, ma_pt> rejected;
  std::vector<double> vec_ids;
  r.push_back(
      integrated_vector_at(i, j, max_magnitud, min_magnitud, avg_if_obstacle));
  ma_pt start(i, j);
  ...
  std::function<void(ma_pt)> get_vectors = [&](ma_pt p) {
    ...
    if (vectors.count(get_node_id(p)) > 0)
      return;
    if (rejected.count(get_node_id(p)) > 0)
      return;
    // if (is_node(p))
    // {
  ...
  if (avg_if_obstacle && is_obstacle(i, j)) {
    ...
    for (auto d : dirs) {
      ...
      if (is_obstacle(EXPAND_CMP(a)) || !in_bounds(EXPAND_CMP(a)))
        continue;
      sf = a + far_vector_at(EXPAND_CMP(a), false); // + d.first;
      // MA_DEBUG(i,j,170,5, "sf: " << sf << " a: " << a << " is_node: " <<
      // is_node(sf));
      if (is_node(sf)) {
        get_vectors(sf);
      }
  ...
  std::shuffle(vec_ids.begin(), vec_ids.end(),
...
std::complex<double> medial_axis_t::integrated_vector_at(int i, int j,
                                                         double max_magnitud,
                                                         double min_magnitud,
                                                         bool avg_if_obstacle) {
  prx_assert(min_magnitud < max_magnitud,
             "Problem with the requested magnitud range of the vector: [ "
                 << min_magnitud << " , " << max_magnitud << " ]");
  ...
  // std::function<std::complex<double>(ug_node_ptr, std::complex<double>)>
  // update_v = [&](ug_node_ptr n, std::complex<double> v)
  // {
  ...
  std::function<std::complex<double>(std::complex<double>)> clamp =
      [&](std::complex<double> uv) {
        double m = std::abs(uv);
        std::complex<double> r;
        if (m == 0) {
  ...
  std::function<std::complex<double>(std::complex<double>)> fancy_grow =
      [&](std::complex<double> v) {
        ...
        while (std::abs(r) < min_magnitud && limit < min_magnitud * 2) {
          point = init + r;
          if (is_obstacle(EXPAND_CMP(point)))
            break;
          if (!in_bounds(EXPAND_CMP(point)))
            break;
          // std::cout << init << " r: " << r;
          // std::cout << " magnitud: " << std::abs(r) << " point: " << point;
          // std::cout << " map_far: " << map_far(EXPAND_CMP(point)) <<
          // std::endl;
  ...
  while (true) {
    // MA_DEBUG(i,j, 15, 60, "u: " << u << " mult: " << mult << "
    // last_good_mult: " << last_good_mult << " best_cost: " << best_cost);
    if (near_ma(u)) //&& !near_ma(u + start) )
    {
      node = graph.get_vertex_as<undirected_node_t>(get_node_id(EXPAND_CMP(u)));

      if (node != nullptr && mult > 0 &&
          best_cost >= node->get_cost_to_go() + mult) {
        best_cost = std::ceil(mult + node->get_cost_to_go());
        last_good_mult = mult; // break;
      }
    }
    // if (mult >= max_magnitud ) break;
    if (std::abs(goal_c - u) <= 1) {
      last_good_mult = mult;
      break;
    }
    // MA_DEBUG(i, j, 14, 6, "u: " << u << " was_near_ma: " << near_ma(u) << "
    // near_ma: " << near_ma(u + start)); if ( was_near_ma &&
    // !near_ma(std::floor(u.real()), std::floor(u.imag())) ) break;
    if (!in_bounds(std::floor(u.real()), std::floor(u.imag())))
      break; // return clamp(start * last_good_mult);
    if (is_obstacle(std::floor(u.real()), std::floor(u.imag())))
      break; // return clamp(start * last_good_mult);
    // if ( best_cost != std::numeric_limits<double>::infinity() &&
    // get_dist_to_ma(EXPAND_CMP(u)) > dist_to_obstacle(u)) break; if (
    // best_cost != std::numeric_limits<double>::infinity() &&
    // std::min(get_dist_to_ma(EXPAND_CMP(u)), los_clearance) >
    // dist_to_obstacle(u) ) break; if ( std::min(mult, los_clearance) >
    // dist_to_obstacle(u) ) break;
    if (mult > los_clearance &&
        std::min(los_clearance, get_dist_to_ma(EXPAND_CMP(u))) >
            dist_to_obstacle(u))
      break;
  ...
  mult = best_cost == std::numeric_limits<double>::infinity() ? mult
                                                              : last_good_mult;
...
void medial_axis_t::vector_field_from_file(std::string file_name, int map) {
  ...
  while (std::getline(ifs, line)) {
    ...
    if (map == 0)
      map_close(w, h) = std::complex<double>(v1, v2);
    else if (map == 1)
      map_far(w, h) = std::complex<double>(v1, v2);
  }
}
...
void medial_axis_t::integrated_vf_to_file(std::string file_name) {
  ...
  for (int i = 0; i < rows(); ++i) {
    for (int j = 0; j < cols(); ++j) {
      ...
      // uv = integrated_vector_at(i,j, std::numeric_limits<double>::max(),
      // false); uv = compute_integrated_vector(i, j);
      ofs_map << i << " " << j << " ";
      ofs_map << uv.real() << " " << uv.imag() << std::endl;
      // }
    }
...
void medial_axis_t::blossom_to_file(std::string file_name) {
  ...
  for (int i = 0; i < rows(); ++i) {
    for (int j = 0; j < cols(); ++j) {
      ...
      if (uniform_random() < .99)
        continue;
      auto vs = blossom(i, j, 5, 100, 10, true);
      prx_assert(vs.size() > 1 || is_obstacle(i, j) || !in_bounds(i, j),
                 "returning only one vec:  ( " << i << ", " << j << " )");
      for (auto v : vs) {
        ofs_map << i << " " << j << " ";
...
bool medial_axis_t::has_direct_line_of_sight(space_point_t init,
                                             space_point_t end,
                                             double clearance) {
  ...
  std::function<bool(space_point_t)> is_in_bounds = [&](space_point_t p1) {
    return p1->at(0) >= 0 && p1->at(1) >= 0 && p1->at(0) < rows() &&
           p1->at(1) < cols();
  };
  ...
  for (double i = delta; i <= 1; i += delta) {
    ...
    // MA_DEBUG(std::floor(init -> at(0)), std::floor(init -> at(1)), 160, 160,
    // "\tpt_cl1: " << state_space -> print_point(pt_cl1, 2) )

    min_clearance = std::min(
        clearance, min_dist); // clearance < min_dist ? clearance : min_dist;
    double dist_to_obst = dist_to_obstacle(std::complex<double>(
        std::floor(pt_cl1->at(0)), std::floor(pt_cl1->at(1))));
    if (dist_to_obst < 1.)
      continue;

    if (dist_to_obst < min_clearance)
      return false;
  }
  return true;
}
...
void medial_axis_t::compute_distances_to_ma() {
  std::function<void(int, int, double)> check_and_add_to_queue =
      [&](int n, int m, double dist) {
