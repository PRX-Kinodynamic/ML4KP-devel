...
void medial_axis_t::set_map(int x_grid, int y_grid, valid_state_t& valid_state, mapping_f f, space_t* state_space,
...
void medial_axis_t::find_medial_axis()
{
  ...
  auto add_node_to_graph = [&](ma_pt pt) {
    ...
    auto node = graph.get_vertex_as<undirected_node_t>(node_index);
    node->point = state_space->make_point();
    state_space->copy(node->point, { pt.real(), pt.imag() });
  ...
  if (avg_if_obstacle && is_obstacle(i, j))
  {
    ...
    for (auto d : dirs)
    {
      ...
      if (is_obstacle(EXPAND_CMP(a)) || !in_bounds(EXPAND_CMP(a)))
        continue;
      sf = a + far_vector_at(EXPAND_CMP(a), false);
      if (is_node(sf))
      {
        get_vectors(sf);
      }
