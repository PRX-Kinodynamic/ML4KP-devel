...
constexpr std::complex<double> ma_cell::
    z_obstacle;  // = std::complex<double>(std::numeric_limits<double>::max(),std::numeric_limits<double>::max());
...
void medial_axis_t::find_medial_axis()
{
  ...
  std::function<bool(int, int)> is_ma = [&](int i, int j) {
    ...
    // if (0 <= i-1 && 0 <= j-1 && i-1 < rows() && j-1 < cols() && !is_obstacle(i-1, j-1))
    // vs.push_back(-map_close(i-1,j-1));
    ...
    // if (0 <= i+0 && 0 <= j+0 && i+0 < rows() && j+0 < cols() && !is_obstacle(i+0, j+0))
    // vs.push_back(-map_close(i+0,j+0));
    if (0 <= i + 0 && 0 <= j + 1 && i + 0 < rows() && j + 1 < cols() && !is_obstacle(i + 0, j + 1))
      vs.push_back(-map_close(i + 0, j + 1));
    // if (0 <= i+1 && 0 <= j-1 && i+1 < rows() && j-1 < cols() && !is_obstacle(i+1, j-1))
    // vs.push_back(-map_close(i+1,j-1));
    ...
    {
      // MA_DEBUG(i, j, 181, 350, "c: " << c << " vs[k]: " << vs[k] << " angle: " << ang << " count: " <<
      ...
      // // 	else if (std::abs(std::acos(dot(vs[k], vs[l]) / (std::abs(vs[k]) * std::abs(vs[l])))) >= M_PI / 2.0 )
      // return true; // std::abs(std::arg(vs[i] + vs[j]));
      // }
    }
  ...
  std::function<void(ma_pt, double)> set_MA = [&](ma_pt pt, double dist_prev) {
    ...
    // // MA_DEBUG(i,j, 10, 10, "curr_angle: " << curr_angle << " dist_prev: " << dist_prev << " distance: " <<
    // angle_diff(curr_angle, dist_prev));
    ...
    // std::cout << "\tpt: " << pt << " curr_angle: " << curr_angle << " close_ang: " << close_ang << " diff: " <<
    // angle_diff(curr_angle, close_ang) << std::endl; auto cv = pt + close ;// close_vector_at(EXPAND_CMP(pt));
    ...
    for (auto d : dir4)
    {
      ...
      auto v = adj + close;
      // std::cout << "\tclose: " << close << " v: " << v << " diff: " << std::fabs(std::abs(first_obst) - std::abs(v))
      // << std::endl; if (first)
      // {
      // 	first = false;
      // 	first_obst = v;
...
void medial_axis_t::compute_far_vector(int i, int j)
{
  ...
  else  // Find the medial axis node with direct line of sight from current pos that is closest to the goal
  {
    ...
    for (auto n : vec_nodes)
    {
      ...
      //  " cost: " << node -> get_cost_to_go() << " best_cost: " << best_cost << " dist: " << df_ma(pt, node ->
      //  point));
  ...
  std::function<void(ma_pt, ma_pt)> add_to_cells = [&](ma_pt pt, ma_pt obstacle) {
    ...
    for (auto d : dir4)
    {
      ...
      if (in_bounds(EXPAND_CMP(a)) && !is_obstacle(a.real(), a.imag()) && dist_to_obstacle(a) > (dist_aux + d.second))
      {
        ...
        // std::cout << "pt: " << pt << " pt_dist: " << dist_to_obstacle(pt) << "cell: " << a << " dist: " <<
        // dist_to_obstacle(a) << std::endl; ma_map[a.real()][a.imag()].close_count++;
                                                         ...
                                                         bool avg_if_obstacle)
{
  ...
  // std::function<std::complex<double>(ug_node_ptr, std::complex<double>)> update_v = [&](ug_node_ptr n,
  // std::complex<double> v)
  // {
  ...
  while (true)
  {
    // MA_DEBUG(i,j, 15, 60, "u: " << u << " mult: " << mult << " last_good_mult: " << last_good_mult << " best_cost: "
    // << best_cost);
    ...
    if (is_obstacle(std::floor(u.real()), std::floor(u.imag())))
      break;  // return clamp(start * last_good_mult);
    // if ( best_cost != std::numeric_limits<double>::infinity() && get_dist_to_ma(EXPAND_CMP(u)) > dist_to_obstacle(u))
    // break; if ( best_cost != std::numeric_limits<double>::infinity() && std::min(get_dist_to_ma(EXPAND_CMP(u)),
    // los_clearance) > dist_to_obstacle(u) ) break; if ( std::min(mult, los_clearance) > dist_to_obstacle(u) ) break;
...
bool medial_axis_t::has_direct_line_of_sight(space_point_t init, space_point_t end, double clearance)
{
  ...
  for (double i = delta; i <= 1; i += delta)
  {
    ...
    // MA_DEBUG(std::floor(init -> at(0)), std::floor(init -> at(1)), 160, 160, "\tpt_cl1: " << state_space ->
    // print_point(pt_cl1, 2) )
