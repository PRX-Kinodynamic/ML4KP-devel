...
void tree_t::to_file(const std::string filename, const std::ios_base::openmode _mode) const
{
  std::ofstream ofs_map;
  ofs_map.open(filename.c_str(), _mode);

  std::cout << "TREE TO FILE";
  for (auto e : edge_list)
  {
    std::cout << "i: " << e->index << "\n";
    ofs_map << e->index << " ";
    ofs_map << e->source << " ";
    ofs_map << e->target << " ";
    ofs_map << v_index_map[e->source]->point << " ";
    ofs_map << v_index_map[e->target]->point;
    ofs_map << "\n";
  }
  ofs_map.close();
}
