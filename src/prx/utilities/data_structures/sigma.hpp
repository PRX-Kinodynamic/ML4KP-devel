 ...
 */
template <typename T> class sigma_r_t {
public:
  ...
  std::shared_ptr<sigma_r_t<T>> get_parent() { return parent; }

  int get_r_increase() { return r_increase; }

  int get_r() { return r; }

  std::vector<int> get_equiv_class() { return equiv_class; }
  ...
  friend std::ostream &operator<<(std::ostream &os, const sigma_r_t &obj) {
    ...
    for (auto eqcl : obj.sigma_r) {
      os << eqcl.second->node << "," << eqcl.first << ","
         << *(eqcl.second->point) << std::endl;
    }
    // os << std::endl;
    return os;
  }
...
public:
  ...
  /**
   * @brief Finds pt in Sigma. Returns (true, index) if found, (false, 0) if not
   * found.
   * @details Look for pt in the Sigma set. Returns a pair of (Found,
   * node_index), where found is false if there is no node for that equivalent
   * class or true and the node that represents that equivalent class.
   *
  ...
  std::pair<bool, T> find(space_point_t pt, int r) {
    prx_assert(state_space->is_point_in_space(pt),
               "Point of different state space");
  ...
  void replace(T n_new, space_point_t pt_new, space_point_t pt_old, int r) {
    prx_assert(state_space->is_point_in_space(pt_new),
               "Point of different state space");
    if (pt_old != nullptr)
      prx_assert(state_space->is_point_in_space(pt_old),
                 "Point of different state space");
    std::shared_ptr<sigma_r_t<T>> s_r = root;
    std::vector<int> eqcl;
    std::string eqcl_str;
    ...
    do {
      ...
      if (s_r->sigma_r[eqcl_str] == nullptr) {
        s_r->sigma_r[eqcl_str] =
            std::make_shared<sigma_r_t<T>>(s_r, n_new, pt_new);
  ...
  void set_sigma_limit(const int _sigma_limit) { sigma_limit = _sigma_limit; }
...
protected:
  ...
  std::string equiv_class_as_string(std::vector<int> equiv_class) {
    ...
    for (auto p : equiv_class) {
      ...
      first = false;
      if (p ==
          -0) // Necessary to handling the case of negative zero ==> -0 == 0
      {
        str += std::to_string(0);
      } else {
        str += std::to_string(p);
