...
{
public:
  sigma_t(eta_function_t eta_f, std::shared_ptr<space_t> _state_space, space_point_t initial_state, int r_init,
          int r_increase)
  {
    eta = eta_f;
  ...
  void replace(T n_new, space_point_t pt_new, space_point_t pt_old, int r)
  {
    ...
    if (pt_old != nullptr)
      prx_assert(state_space->is_point_in_space(pt_old), "Point of different state space");
    std::shared_ptr<sigma_r_t<T>> s_r = root;
    std::vector<int> eqcl;
    std::string eqcl_str;
    int i = root->get_r();
