...
public:
  ...
  cube_cell_t(const Coordinate& min_vertex, const CoordinatePtr& lengths, Args... args)
    : _min_vertex(min_vertex), _lengths(lengths), _element(args...){};

  template <typename... Args>
  cube_cell_t(const Coordinate& min_vertex, const Coordinate& lengths, Args... args)
    : cube_cell_t(min_vertex, std::make_shared<Coordinate>(lengths), args...)
  {
  }
...
private:
  ...
  void allocate_all(Args... args)
  {
    ...
    do
    {
      ...
      _grid[idx] = std::make_shared<CellType>(x, _cell_length, args...);
...
public:
  template <typename ElementToCellFunction, typename... Args>
  regular_grid_t(const std::string filename, ElementToCellFunction func, Args... args)
    : regular_grid_t(from_file(filename, func, args...))
  {
  }

  template <typename... Args>
  regular_grid_t(const Coordinate min, const Coordinate max, const Coordinate cell_length, Args... args)
    : _min(min)
    , _max(max)
  ...
  template <typename ElementToCellFunction, typename... Args>
  static Grid from_file(const std::string filename, ElementToCellFunction func, Args... args)
  {
    ...
    prx_assert(dim == Dimension, "Wrong dimension");
    ...
    Grid grid(min, max, cell_length, args...);
