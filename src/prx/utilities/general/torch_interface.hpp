...
public:
  ...
  OutType operator()(const InTypes&... types)
  {
    ...
    // prx::copy(_output, tensor_output);
...
protected:
  static ModulePtr load_model_(const std::string filename)
  {
    ...
    {
      std::cout << e.what() << "\n";
      prx_throw("Error loading the model\n");
    }
    return module;
  }
  ...
  {
    return gtsam::traits<Car>::dimension;
  }
  ...
  static void fill_inputs(Type& type, Inputs& inputs, TensorArray& tensor_array, const std::size_t idx)
  {
    ...
    prx::copy(tensor_array[idx], type);
                                  ...
                                  GradientsArray& grads, const std::size_t& row, const std::size_t& col)
  {
    ...
    // PRX_DBG_VARS(H);
    prx::copy(H.block(row, 0, 1, H.cols()).transpose(), partial[0]);
    // PRX_DBG_VARS(H);
