...
std::vector<double> statistics_t::sample_mean()
{
  ...
  std::transform(sum.begin(), sum.end(), sum.begin(), [n](double s) -> double { return s / n; });
  return sum;
}
...
std::vector<double> statistics_t::sample_variance()
{
  ...
  std::vector<std::vector<double>> sums = { std::vector<double>(samples[0].size(), 0.0),
                                            std::vector<double>(samples[0].size(), 0.0) };
...
std::vector<double> statistics_t::sample_standard_deviation()
{
  ...
  std::transform(sv.begin(), sv.end(), sv.begin(), [](double s) -> double { return std::sqrt(s); });
  return sv;
}
