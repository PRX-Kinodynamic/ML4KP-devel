...
namespace prx
{
	...
	public:
		...
		static
		prx_symbol_t create_symbol(std::string name, Ts... args)
		{
 ...
 * @param      SYMBOL_STR   Maximum of two-char string to use as variable identifier. If an empty string is used, the symbol is constructed from the corresponding parameter suplied to symbol_factory_t::create_symbol
 * @param      SYMBOL_IDX   Symbol numeric identifier. If a 0 is used, the symbol is constructed from the corresponding parameter suplied to symbol_factory_t::create_symbol
 * @param      SYMBOL_T     The time symbol. . If a 0 is used, the symbol is constructed from the corresponding parameter suplied to symbol_factory_t::create_symbol
 *
...
{ \
	static auto FN_##SYMBOL_NAME##_GENERATOR = [](const std::string& _s, uint8_t _state_idx, uint64_t _t) \
	{ \
		const std::string str 	=  _s 			== "" ? SYMBOL_STR  : _s		;\
		const uint8_t idx 		= _state_idx 	== 0  ? SYMBOL_IDX  : _state_idx; \
		const uint64_t t 		= _t 			== 0  ? SYMBOL_T 	: _t 		 ;\
        return prx_symbol_t(str, idx, t);\
	}; \
	const bool VAR_##SYMBOL_NAME##_REGISTRED = symbol_factory_t::get().register_symbol(#SYMBOL_NAME, FN_##SYMBOL_NAME##_GENERATOR); \
} }
