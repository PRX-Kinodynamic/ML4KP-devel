...
namespace prx
{
bullet_collision_group_t::bullet_collision_group_t(const std::vector<system_ptr_t>& in_plants,
...
bool bullet_collision_group_t::is_contact_excluded(const b3ContactPointData& contact)
{
  for (auto i : collision_exclusion_list)
  {
    ...
    {
      if (i.second.first == contact.m_bodyUniqueIdB &&
          (i.second.second == contact.m_linkIndexB || i.second.second == -1))
      {
        return true;
      }
    }
