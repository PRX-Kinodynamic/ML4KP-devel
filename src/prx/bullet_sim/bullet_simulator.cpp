...
bullet_simulator_t::bullet_simulator_t() : simulator_t(plant_type::BULLET)
{
...
void bullet_simulator_t::initialize_simulation()
{
  ...
  for (auto f : urdf_paths)
  {
    ...
    if (!f.second)
      allowed_collisions.push_back(body_id);
  }
...
{
  prx_assert(this->canSubmitCommand(),
             "Error with bullet simulation: Cannot submit command (bullet_simulator_t::canSubmitCommand)");
