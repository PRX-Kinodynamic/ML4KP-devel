#include "prx/planning/heuristics/medial_axis.hpp"
...
void medial_axis_t::add_obstacles_to_graph()
{
  ...
  std::function<bool(int, int)> surrounded_by_obstacles = [&](int i, int j) {
    ...
    auto vec = { std::make_pair(i - 1, j - 1), std::make_pair(i - 1, j),    std::make_pair(i - 1, j + 1),
                 std::make_pair(i, j - 1),     std::make_pair(i, j + 1),    std::make_pair(i + 1, j - 1),
...
void medial_axis_t::set_sknw(const std::string nodes_file, const std::string edges_file, const std::string sknw_file)
{
  ...
  for (std::string line; std::getline(ifs_s, line);)
  {
    ...
    std::function<void(node_index_t, node_index_t, int, int)> add_new_node = [&](node_index_t n1, node_index_t n2,
                                                                                 int edge1_index, int edge2_index) {
...
void medial_axis_t::compute_sknw_and_save_to_files(std::string nodes_file, std::string edges_file,
                                                   std::string sknw_file, std::string py_sknw, std::string py_cmd)
{
  prx_assert(goal != nullptr, "Goal not set!");

  std::string cmd =
      py_cmd + " " + py_sknw + " " + map_file_name + " " + nodes_file + " " + edges_file + " " + sknw_file;
...
void medial_axis_t::find_medial_axis()
{
  ...
  std::function<double(ma_pt, ma_pt)> dot = [](ma_pt a, ma_pt b) { return a.real() * b.real() + a.imag() * b.imag(); };
  ...
  std::function<bool(int, int)> is_ma = [&](int i, int j) {
    ...
    if (0 <= i - 1 && 0 <= j + 0 && i - 1 < rows() && j + 0 < cols() && !is_obstacle(i - 1, j + 0))
      vs.push_back(-map_close(i - 1, j + 0));
    // if (0 <= i-1 && 0 <= j+1 && i-1 < rows() && j+1 < cols() &&
    // !is_obstacle(i-1, j+1)) vs.push_back(-map_close(i-1,j+1));
    if (0 <= i + 0 && 0 <= j - 1 && i + 0 < rows() && j - 1 < cols() && !is_obstacle(i + 0, j - 1))
...
void medial_axis_t::compute_close_vector_field()
{
  ...
  std::function<void(ma_pt, ma_pt)> add_to_cells = [&](ma_pt pt, ma_pt obstacle) {
    ...
    for (auto d : dir4)
    {
      ...
      if (in_bounds(EXPAND_CMP(a)) && !is_obstacle(a.real(), a.imag()) && dist_to_obstacle(a) > (dist_aux + d.second))
      {
...
std::vector<std::complex<double>> medial_axis_t::blossom(int i, int j, int n, double max_magnitud, double min_magnitud,
...
std::complex<double> medial_axis_t::integrated_vector_at(int i, int j, double max_magnitud, double min_magnitud,
                                                         bool avg_if_obstacle)
{
  prx_assert(min_magnitud < max_magnitud, "Problem with the requested magnitud range of the vector: [ "
  ...
  while (true)
  {
    ...
    if (mult > los_clearance && std::min(los_clearance, get_dist_to_ma(EXPAND_CMP(u))) > dist_to_obstacle(u))
      break;
...
bool medial_axis_t::has_direct_line_of_sight(space_point_t init, space_point_t end, double clearance)
{
  ...
  std::function<bool(space_point_t)> is_in_bounds = [&](space_point_t p1) {
    return p1->at(0) >= 0 && p1->at(1) >= 0 && p1->at(0) < rows() && p1->at(1) < cols();
  };
