 ...
 */
// static std::function<bool(std::complex<double>,std::complex<double>)>
// cmpx_comp = [] (std::complex<double> a, std::complex<double> b)
// 	{
...
public:
  static constexpr std::complex<double> z_obstacle = std::complex<double>(
...
private:
   ...
   *
   * Save to files: integrated_vf_to_file, close_vf_to_file, far_vf_to_file,
   * grap_vertices_to_file, gnn_to_file
   *
   */
  enum Stage {
    instatiated,
  ...
  // map_close; Eigen::Matrix <std::complex<double>, Eigen::Dynamic,
  // Eigen::Dynamic> map_far; ma_map_t* ma_map;
  ...
  std::vector<std::pair<ma_pt, double>> dirs = {
      std::pair<ma_pt, double>(std::complex<double>(+1, +0),
                               std::abs(std::complex<double>(+1, +0))),
      std::pair<ma_pt, double>(std::complex<double>(+1, +1),
                               std::abs(std::complex<double>(+1, +1))),
      std::pair<ma_pt, double>(std::complex<double>(+0, +1),
                               std::abs(std::complex<double>(+0, +1))),
      std::pair<ma_pt, double>(std::complex<double>(-1, +1),
                               std::abs(std::complex<double>(-1, +1))),
      std::pair<ma_pt, double>(std::complex<double>(-1, +0),
                               std::abs(std::complex<double>(-1, +0))),
  ...
  std::complex<double> &map_far(int i, int j) { return ma_map[i][j].far; }
  const std::complex<double> map_far(int i, int j) const {
    return ma_map[i][j].far;
  }

  std::complex<double> &map_close(int i, int j) { return ma_map[i][j].close; }
  ...
  void set_as_edge(int i, int j, int edge_id) {
    if (ma_map[i][j].close == ma_cell::z_obstacle)
      return;
    if (ma_map[i][j].is_node)
      return;
    ma_map[i][j].is_edge = true;
    ma_map[i][j].dist_to_ma = 0;
  ...
  double get_dist_to_ma(int i, int j) { return ma_map[i][j].dist_to_ma; }
  ...
  int get_edge_id(int i, int j) { return ma_map[i][j].edge_id; }

  bool is_node(ma_pt v) {
    if (v.real() < 0 || v.real() >= rows())
      return false;
    if (v.imag() < 0 || v.imag() >= cols())
      return false;
  ...
  bool is_goal(double i, double j) {
    ...
    if (id1 == goal_index || id2 == goal_index)
      printf("GOAL!\n");
    return id1 == goal_index || id2 == goal_index;
  }

  inline node_index_t get_node_id(int i, int j) { return ma_map[i][j].node_id; }
...
public:
  ...
  medial_axis_t() {
    ...
    v_not_computed = std::complex<double>(std::numeric_limits<double>::min(),
                                          std::numeric_limits<double>::min());

    state_memory = {&h, &w};
    state_space = new space_t("EE", state_memory, "HW");
    state_space->set_bounds({-std::numeric_limits<double>::max(),
  ...
  void init() {
    prx_assert(df_ma != nullptr,
               "distance_function not set for medial_axis_t!");
  ...
  void set_sknw(
      const std::string nodes_file, const std::string sknw_file,
      const std::string edges_file); // Should get skeletonization within c++
  ...
  blossom(int i, int j, int n,
          double max_magnitud = std::numeric_limits<double>::max(),
          double min_magnitud = 0.0, bool avg_if_obstacle = false);

  /**
   * Compute the integrated vector at (i,j)
   * The magnitud m of the output vector is such that: m \in [min_magnitud,
   * max_magnitud] The default values are [0, \inf] Notice that if 0 <
   * min_magnitud the resulting vector could be in collision.
   */
  std::complex<double>
  integrated_vector_at(int i, int j,
                       double max_magnitud = std::numeric_limits<double>::max(),
                       double min_magnitud = 0.0, bool avg_if_obstacle = false);
  ...
  void compute_sknw_and_save_to_files(
      std::string nodes_file, std::string edges_file, std::string sknw_file,
      std::string py_sknw = lib_path + "/scripts/medial_axis.py",
  ...
  template <typename valid_state_t>
  void set_map(int x_grid, int y_grid, valid_state_t &valid_state, mapping_f f,
               space_t *state_space,
               std::string _map_name = lib_path + "/out/ma_custom.map") {
