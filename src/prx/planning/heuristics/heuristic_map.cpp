...
#include "prx/planning/heuristics/heuristic_map.hpp"
namespace prx
{
heuristic_map_t::heuristic_map_t(const double world_origin[2], const double _goalx, const double _goaly,
                                 const int grid_h, const int grid_w, const double c_size, const int _knn,
  ...
  for (int i = 0; i < grid_length; i++)
  {
    for (int j = 0; j < grid_width; j++)
    {
      if (i == 0 || i == grid_length - 1 || j == 0 || j == grid_length - 1)
      {
        ...
        std::uniform_real_distribution<double> distribution(0.25 * std::min(grid_width, grid_length),
                                                            std::min(grid_width, grid_length));
        ...
        for (int x = x_min; x <= x_max; x++)
        {
          ...
          for (int y = y_min; y <= y_max; y++)
          {
            ...
            // seen_map[x][y] = true;
            global_cost_map[x][y] = std::max(
                global_cost_map[x][y], global_cost_map[i][j] * exp(-((d - mu) * (d - mu) / (2.0 * sigma * sigma))));
          }
        }
      }
    }
...
{
  // grid_length = 22.2 / c_size + 22; // added extra space for local map
  // collection - 21 extra cells in each direction for 41x41 local map
  // grid_width = 22.2 / c_size + 22;
  grid_length = 22.2 / c_size + 51;  // added extra space for local map collection - 21
                                     // extra cells in each direction for 41x41 local map
...
/*
heuristic_map_t::heuristic_map_t(double origin[2], double goal_pos[2], double
c_size)
{
        grid_length = 100;
        grid_width = 100;
        ...
        for(int i = 0; i < grid_length; i++)
        {
                global_cost_map[i] = (double *) malloc(grid_width *
sizeof(double)); global_h_map[i] = (double *) malloc(grid_width *
sizeof(double)); seen_map[i] = (bool *) malloc(grid_width * sizeof(bool));
                for(int j = 0; j < grid_width; j++)
                {
                        global_cost_map[i][j] = -1;
                        seen_map[i][j] = false;
...
void heuristic_map_t::process_map(double** new_map, double origin[2], int map_l, int map_w, const double _goalx,
                                  const double _goaly, double c_size)
{
  ...
  if (map_l != grid_length || map_w != grid_width)
  {
    ...
    for (int i = 0; i < map_l; i++)
    {
      ...
      {
        if (new_map[i][j] == PRX_UNSEEN_CELL)
          new_global_cost_map[i][j] = new_map[i][j];
        else
          new_global_cost_map[i][j] = new_map[i][j];
    ...
    // dl = ((origin[0]-(map_l/2.0*cell_size)) - delta_x);		//
    // (X_local_bleft-X_global_bleft)*cell_size dw =
    // ((origin[1]-(map_w/2.0*cell_size)) - delta_y);
    int dl = (origin[0] - (delta_x + grid_length / 2.0)) * cell_size + grid_length / 2.0 -
             map_l / 2.0;  // (X_local_bleft-X_global_bleft)*cell_size
    int dw = (origin[1] - (delta_y + grid_width / 2.0)) * cell_size + grid_width / 2.0 - map_w / 2.0;
    std::cout << "bleft local: " << (origin[0] - (map_l / 2.0 * cell_size)) << "\t"
              << (origin[1] - (map_w / 2.0 * cell_size)) << std::endl;
    std::cout << "New dl, dw: " << dl << "\t" << dw << std::endl;
    // print_cost_map();
  }
  // dl = (origin[0] - (delta_x+grid_length / 2.0)) * cell_size +
  // grid_length/2.0 - map_l/2.0*cell_size;		//
  // (X_local_bleft-X_global_bleft)*cell_size dw = (origin[1] -
  // (delta_y+grid_width / 2.0)) * cell_size + grid_width/2.0 -
  // map_w/2.0*cell_size;
  else
  {
    ...
    for (int i = 0; i < map_l; i++)
    {
      ...
      {
        if (new_map[i][j] == PRX_UNSEEN_CELL)
          global_cost_map[i][j] = new_map[i][j];
        else
          global_cost_map[i][j] = new_map[i][j];
...
void heuristic_map_t::init_env(std::vector<std::shared_ptr<movable_object_t>>& obstacle_list,
  ...
  for (int i = 0; i < grid_length; i++)
  {
    for (int j = 0; j < grid_width; j++)
    {
      ...
      if (context.second->in_collision())
      {
        ...
        int obs_area_width = obs_area_length;
        // Fill buckets
        int x_min = std::round(std::max(0, i - obs_area_length / 2)),
            x_max = std::round(std::min(grid_length - 1, i + obs_area_length / 2));
        for (int x = x_min; x <= x_max; x++)
        {
          int y_min = std::round(std::max(0, j - obs_area_width / 2)),
              y_max = std::round(std::min(grid_width - 1, j + obs_area_width / 2));
          for (int y = y_min; y <= y_max; y++)
          {
            if (x == i && y == j)
              continue;
            ...
            double d = sqrt((i - x) * (i - x) + (j - y) * (j - y));
            global_cost_map[x][y] =
                std::max(global_cost_map[x][y], exp(-((d - mu) * (d - mu) / (2.0 * sigma * sigma))));
          }
        }
      }
    }
...
double** heuristic_map_t::init_rand_map(double _start[2], int dim[2], double dx, double dy, double p_density)
{
  ...
  for (int i = 0; i < dim[0]; i++)
  {
    ...
    for (int j = 0; j < dim[1]; j++)
    {
      ...
      if (seen_map[global_map[0]][global_map[1]])
        cost_map[i][j] = global_cost_map[global_map[0]][global_map[1]];
      else
      {
  ...
  for (int i = 0; i < dim[0]; i++)
  {
    for (int j = 0; j < dim[1]; j++)
    {
      ...
      if ((std::abs(i - startx) < 1 / cell_size && std::abs(j - starty) < 1 / cell_size) ||
          (std::abs(j - goaly) < 1 / cell_size && std::abs(i - goalx) < 1 / cell_size))
        continue;
      if (((double)std::rand() / (RAND_MAX)) < obstacle_frequency)
      {
        ...
        cost_map[i][j] = 1.0;
        int obs_area_length;  // = std::rand() % std::min(dim[1], dim[0]);
                              // // area affected by terrain near obstacle
        if (cost_map[i][j] == 1)
        {
        ...
        // std::cout << i << " " << j << " " << cost_map[i][j] << " " <<
        // obs_area_length << std::endl; int obs_area_length = std::rand() %
        // dim[0]; int obs_area_width = std::rand() % dim[1];
        int obs_area_width = obs_area_length;
        // Fill buckets
        ...
        for (int x = x_min; x < x_max; x++)
        {
          ...
          for (int y = y_min; y < y_max; y++)
          {
            ...
            double d = sqrt((i - x) * (i - x) + (j - y) * (j - y));
            cost_map[x][y] =
                std::max(cost_map[x][y], cost_map[i][j] * exp(-((d - mu) * (d - mu) / (2.0 * sigma * sigma))));
          }
        }
      }
    }
  ...
  {
    for (int j = std::max(0, (int)(starty - 1.0 / cell_size)); j < std::min(dim[1], (int)(starty + 1.0 / cell_size));
         j++)
    {
      int local[2] = { i, j };
      int global_map[2];
  ...
  // // Free goal area
  // for(int i = std::max(0, (int) (local_goal[0] - 1.0/cell_size)); i <
  // std::min(dim[0], (int) (local_goal[0] + 1.0/cell_size)); i++)
  // {
  // 	for(int j = std::max(0, (int) (local_goal[1] - 1.0/cell_size)); j <
  // std::min(dim[1], (int) (local_goal[1] + 1.0/cell_size)); j++)
  // 	{
  // 		int local[2] = {i, j};
  // 		int global_map[2];
  // 		local_map_to_global_map_space(global_map, local, dim, dx, dy);
...
bool heuristic_map_t::is_collision_free(const space_point_t point)
{
  ...
  int g_map[2];
  global_to_global_map_space(g_map, global);
  // std::cout<< global_cost_map[g_map[0]][g_map[1]]<< "\t"<<in_grid(g_map[0],
  // g_map[1])<<"\t"<<(global_cost_map[g_map[0]][g_map[1]]!=PRX_UNSEEN_CELL)<<std::endl;
  return in_grid(g_map[0], g_map[1]) && global_cost_map[g_map[0]][g_map[1]] != 1 &&
         global_cost_map[g_map[0]][g_map[1]] != PRX_UNSEEN_CELL;
}
...
std::vector<double> heuristic_map_t::get_local_map(double _x, double _y, int size)
{
  ...
  for (int i = x - ((int)(size / 2)); i <= x + ((int)(size / 2)); i++)
  {
    ...
    {
      prx_assert((i < grid_length && i >= 0 && j < grid_width && j >= 0),
                 "Error in local map boundaries: " + std::to_string(i) + " " + std::to_string(j) + " " +
...
double heuristic_map_t::add_time_multiplier(double cost, double x, double y)
{
  ...
  prx_assert(!((traversability > 1 || traversability < 0) && traversability != PRX_UNSEEN_CELL),
             "Invalid traversability value found");
...
void heuristic_map_t::dijkstra()
{
  ...
  if (!in_grid(goalx, goaly))
    get_k_nearest_neighbors(fringe);
  else
    fringe = get_neighbors(goalx, goaly);
  ...
  while (!fringe.empty())
  {
    ...
    fringe.pop_back();
    if (seen[v_cur.x][v_cur.y].n != nullptr)
      continue;
    if (use_min_cost)
      global_h_map[v_cur.x][v_cur.y] = v_cur.g;

    seen[v_cur.x][v_cur.y].n = &v_cur;
    std::vector<node_t> neighbors = get_neighbors(v_cur.x, v_cur.y, global_h_map);
    for (node_t n : neighbors)
    {
      // if (seen[n.x][n.y].n != nullptr || global_cost_map[n.x][n.y] == -1)
      // continue;
      ...
      if (global_cost_map[n.x][n.y] == 1.0)
      {
        ...
        if (use_obs_h)
        {
          ...
          global_h_map[n.x][n.y] = std::numeric_limits<double>::max();
        // seen[n.x][n.y].n = &n;
        continue;
      }
      double g_cur;
      // double cur_cost =
      // 0.01*cell_size/1.0*exp(4.6*global_cost_map[n.x][n.y]); // divide by 1.0
      // because max velocity
      double cur_cost = 1;  // divide by 1.0 because max velocity
      // if (use_learning) cur_cost = .99*cell_size/1.0;

      // warning: expression result unused [-Wunused-value]
      // if (global_cost_map[n.x][n.y] == PRX_UNSEEN_CELL) 0.01*cell_size/1.0;
      // // optimistic treatment of unseen areas
  ...
  if (use_obs_h)
  {
    ...
    */
    while (!obs_fringe.empty())
    {
      ...
      std::pop_heap(obs_fringe.begin(), obs_fringe.end());
      obs_fringe.pop_back();
      ...
      for (node_t n : neighbors)
      {
        ...
        if (global_cost_map[n.x][n.y] == 1.0 || global_h_map[n.x][n.y] == std::numeric_limits<double>::max())
        {
          //
          // double g_cur =
          // 0.01*cell_size/1.0*exp(4.6*global_cost_map[n.x][n.y]) +
          // global_h_map[v_cur.x][v_cur.y] + get_num_obs_neighbors(v_cur.x,
          // v_cur.y);
          double g_cur = 1 + global_h_map[v_cur.x][v_cur.y] + get_num_obs_neighbors(v_cur.x, v_cur.y);
          prx_assert((global_h_map[v_cur.x][v_cur.y] != std::numeric_limits<double>::max()), "ugh");
          if (g_cur >= global_h_map[n.x][n.y])
...
{
  // local space
  if (x >= 0 && x < grid_length && y >= 0 && y < grid_width)
    return true;
  else
    return false;
}
...
void heuristic_map_t::print_h_map()
{
  for (int i = grid_length - 1; i >= 0; i--)
  {
    for (int j = 0; j < grid_width; j++)
    {
      if (global_h_map[i][j] == std::numeric_limits<double>::max())
        std::cout << -1 << " ";
      else
        std::cout << global_h_map[i][j] << " ";
    }
    std::cout << std::endl;
  }
  std::cout << std::endl;
...
heuristic_map_t::heuristic_map_t(const double world_origin_x, const double world_origin_y, const double _goalx,
                                 const double _goaly, const int grid_h, const int grid_w, const double c_size,
                                 const std::string map_fname, const int padding)
{
  ...
  for (int i = 0; i < grid_length; i++)
  {
    ...
    for (int j = 0; j < grid_width; j++)
    {
      ...
      if (i < padding || j < padding || i >= grid_length - padding || j >= grid_width - padding)
        global_cost_map[i][j] = 1;
      else if (extracted_map[(j - padding)][(i - padding)])
        global_cost_map[i][j] = 0;
      else
        global_cost_map[i][j] = 1;
  ...
  //                 max_val = max_val > get_score(x,y) || get_score(x,y) ==
  //                 std::numeric_limits<double>::max() ? max_val :
  //                 get_score(x,y);
  //                 // max_val = max_val > global_cost_map[x][y] ||
  //                 global_cost_map[x][y] == std::numeric_limits<double>::max()
  //                 ? max_val : global_cost_map[x][y];
  //         }
  //         // std::cout << std::endl;
  // }
...
bool heuristic_map_t::line_of_sight(int x0, int y0, int x1, int y1)
{
  ...
  if (dx >= dy)
  {
    while (x0 != x1)
    {
      ...
      if (f >= dx)
      {
        if (global_cost_map[x0 + (sx - 1) / 2][y0 + (sy - 1) / 2] == 1)
          return false;
        y0 = y0 + sy;
        f = f - dx;
      }
      if (f != 0 && global_cost_map[x0 + (sx - 1) / 2][y0 + (sy - 1) / 2] == 1)
        return false;
      if (dy == 0 && global_cost_map[x0 + (sx - 1) / 2][y0 - 1] == 1)
        return false;
      x0 += sx;
    }
  }
  else
  {
    while (y0 != y1)
    {
      ...
      if (f != 0 && global_cost_map[x0 + (sx - 1) / 2][y0 + (sy - 1) / 2] == 1)
        return false;
      if (dx == 0 && get_cost(x0, y0 + (sy - 1) / 2) == 1 && get_cost(x0 - 1, y0 + (sy - 1) / 2) == 1)
        return false;
      // if (dx == 0 && global_cost_map[x0][y0+(sy-1)/2] == 1 &&
      // global_cost_map[x0-1][y0+(sy-1)/2] == 1) return false;
      y0 += sy;
    }
  }
  return true;
  ...
  for (int x = 0; x < size; x++)
  {
    for (int y = 0; y < size; y++)
    {
      ...
      while (!fringe.empty())
      {
        ...
        for (node_t n : neighbors)
        {
          if (global_cost_map[n.x][n.y] == -1 || global_cost_map[n.x][n.y] == 1.0 || seen[n.x][n.y].n != nullptr ||
