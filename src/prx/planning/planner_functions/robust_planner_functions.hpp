...
namespace prx
{
    ...
    typedef std::function<void (std::vector<space_point_t>&, plan_t&, std::vector<trajectory_t*>&)> propagate_particles_t;
    ...
    typedef std::function<bool (const std::vector<space_point_t>&)> goal_check_particles_t;
    
    void default_propagate_particles(std::vector<space_point_t>&, plan_t&, std::vector<trajectory_t*>&, std::shared_ptr<system_group_t>);
