...
namespace prx
{
configurable_functions_t::configurable_functions_t(std::string context_name, world_model_context context,
                                                   param_loader pl)
{
  ...
  if (plant_type == "koules")
  {
    ...
    valid_state = [](space_point_t& s) {
      if (s->at(0) < .03 || s->at(0) > 1.0 - .03 || s->at(1) < .03 || s->at(1) > 1.0 - .03)
      {
        return false;
      }
      return true;
  ...
  else if (plant_type == "koules")
  {
    ...
    distance_function = [num_koules](const space_point_t& s1, const space_point_t& s2) {
      double value =
          (s1->at(0) - s2->at(0)) * (s1->at(0) - s2->at(0)) + (s1->at(1) - s2->at(1)) * (s1->at(1) - s2->at(1)) +
          (s1->at(3) - s2->at(3)) * (s1->at(3) - s2->at(3)) + (s1->at(4) - s2->at(4)) * (s1->at(4) - s2->at(4));
  ...
  if (cost_function_type == "distance")
  {
    ...
    h = [this](const space_point_t& s, const space_point_t& s2) { return distance_function(s, s2); };
  }
  else if (cost_function_type == "time")
  {
    cost_function = [](const trajectory_t& t, const plan_t& plan) { return plan.duration(); };
    ...
    {
      velocity_factor = sqrt(s_bounds[3].second * s_bounds[3].second + s_bounds[4].second * s_bounds[4].second);
    }
    else if (plant_type == "mantis")
    {
      velocity_factor = sqrt(s_bounds[4].second * s_bounds[4].second + s_bounds[5].second * s_bounds[5].second +
                             s_bounds[6].second * s_bounds[6].second);
    }
    else if (plant_type == "3d_point")
    {
      velocity_factor = sqrt(c_bounds[0].second * c_bounds[0].second + c_bounds[1].second * c_bounds[1].second +
    ...
    if (plant_type == "koules")
    {
      ...
      h = [this, velocity_factor, num_koules](const space_point_t& s, const space_point_t& s2) {
        double value =
            (s->at(0) - s2->at(0)) * (s->at(0) - s2->at(0)) + (s->at(1) - s2->at(1)) * (s->at(1) - s2->at(1));
