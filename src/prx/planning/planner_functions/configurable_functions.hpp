...
private:
  ...
  double bm_metric_cost(std::vector<double> s1)
  {
    ...
    if (zeta <= s1.at(2))
    {
      return r + std::min(std::fabs(zeta) + std::fabs(zeta - s1.at(2)),
                          2 * PRX_PI + std::fabs(zeta) - std::fabs(zeta + s1.at(2)));
    }
    else if (s1.at(1) <= 1 - std::cos(s1.at(2)))
    {
      if (s1.at(1) == 0)
        return std::fabs(s1.at(0)) + 0.5 * s1.at(2);
      else
        return (s1.at(1) * (1 + std::cos(s1.at(2))) / std::sin(s1.at(2)) - s1.at(0)) + 0.5 * s1.at(2);
    }
