#include "prx/planning/planners/dirt_c2g.hpp"
namespace prx
{

	dirt_c2g_t::dirt_c2g_t(const std::string& new_name) : rrt_t(new_name)
	{
		metric = nullptr;
		child_extension = true;
		max_radius = 0;
		planner_name = "DIRT_C2G";
	}
	dirt_c2g_t::~dirt_c2g_t()
	{
		_reset();
	}
	void dirt_c2g_t::_link_and_setup_spec(planner_specification_t* spec)
	{
		rrt_t::_link_and_setup_spec(spec);
		//reset is always called before this
		dirt_spec = dynamic_cast<dirt_c2g_specification_t*>(spec);
		prx_assert(dirt_spec!=nullptr,"DIRT received an incorrect specification.");

		expand = dirt_spec->expand;
		h = dirt_spec->h;
        batch_h = dirt_spec->batch_h;
	}
	bool dirt_c2g_t::_preprocess()
	{
		tree.allocate_memory<dirt_node_t,rrt_edge_t>(1000);
		return true;
	}
	bool dirt_c2g_t::_link_and_setup_query(planner_query_t* query)
	{
		rrt_query = dynamic_cast<rrt_query_t*>(query);
		prx_assert(rrt_query!=nullptr,"DIRT received an incorrect query type.");
		dirt_query = dynamic_cast<dirt_c2g_query_t*>(query);
	...
	void dirt_c2g_t::_resolve_query(condition_check_t* condition)
	{
		...
		do
		{
			...
			if(closest_node->indices.size()==0)
			{
                ...
                std::vector<space_point_t> end_points;
				int index = 0;
				...
				for(int i=0;i<plans.size();i++)
				{
                    ...
                    end_points.push_back(trajs[i]->back());
                }

                pred_values = batch_h(end_points,dirt_query->goal_state);
			...
			while(closest_node->indices.size()!=0)
			{
                ...
                // The below line works, but the one after doesn't. Weird.
				end_heuristic = h(eg.second->back(),dirt_query->goal_state);
				// end_heuristic = pred_values[closest_node->indices.back()];
	...
	void dirt_c2g_t::add_edge_to_tree(std::pair<plan_t*, trajectory_t*> eg,
	...
	)
	{
		...
		new_tree_node->cost_to_go = node_h;
	...
	std::vector<double> dirt_c2g_t::get_statistics()
	{
	...
	void dirt_c2g_t::bnb(node_index_t v, double cost_bound, bool delete_flag)
	{
