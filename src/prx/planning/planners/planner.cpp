...
bool planner_t::preprocess()
{
  ...
  {
    prx_throw("Preprocess can only be called after link_and_setup_spec or after preprocess. Current planner stage is "
...
{
  if (planner_state != planner_stage_t::PREPROCESS && planner_state != planner_stage_t::LINK_QUERY &&
      planner_state != planner_stage_t::RESOLVE_QUERY && planner_state != planner_stage_t::FULFILL_QUERY)
  {
    prx_throw(
        "Link and setup query can only be after preprocess has been called (any query function). Current planner stage "
        "is "
        << get_current_stage_name());
  }
...
{
  if (planner_state != planner_stage_t::LINK_QUERY && planner_state != planner_stage_t::RESOLVE_QUERY &&
      planner_state != planner_stage_t::FULFILL_QUERY)
  {
    prx_throw(
        "Resolve query can only be called after link_and_setup_query, resolve_query, or fulfill_query. Current planner "
        "stage is "
        << get_current_stage_name());
  }
...
std::string planner_t::get_current_stage_name()
{
  switch (planner_state)
  {
    ...
    default:
      prx_throw("planner_stage_t stage error!") return "ERROR!";
  }
}
}  // namespace prx
