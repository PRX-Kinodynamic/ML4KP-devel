...
namespace prx
{
rrt_t::rrt_t(const std::string& new_name)
  : planner_t(new_name), log_tree(true), tree_logger(prx::out_path + new_name + ".log")
{
  metric = nullptr;
...
void rrt_t::_resolve_query(condition_check_t* condition)
{
      ...
      do
  {
    ...
    if ((goal_vertex == start_vertex || closest_node->cost_to_come + edge_cost < current_solution) && valid_check(traj))
    {
      ...
      if (log_tree)
      {
        tree_logger("A", timer.measure(), closest_node->point, new_tree_node->point);
      }
...
void rrt_t::update_goal(node_index_t node_index)
{
  ...
  if (rrt_query->goal_check(new_tree_node->point))
  {
        ...
        tree.get_vertex_as<rrt_node_t>(goal_vertex)->cost_to_come > new_tree_node->cost_to_come)
    {
      ...
      if (log_tree)
        tree_logger("G", timer.measure(), new_tree_node->point);
...
void rrt_t::bnb(node_index_t v, double cost_bound, bool delete_flag)
{
  ...
  if (res && children.empty())
  {
    if (log_tree)
    {
      auto node_p = tree.get_vertex_as<rrt_node_t>(node->get_parent());
      tree_logger("D", timer.measure(), node->point, node_p->point);
    }
    // remove the node
