...
bool aorrt_t::_link_and_setup_query(planner_query_t* query)
{
  ...
  if (tree.num_vertices() == 0 ||
      !X_state_space->equal_points(tree.get_vertex_as<aorrt_node_t>(start_vertex)->point, aorrt_query->start_state))
  {
    ...
    Y_state_space->point_union(aorrt_query->start_state, cost_state_space->get_start_state(), start_node->point);
...
{
  // Experimentally, we have found that having cost \in [0,\inf] is not
  // the best. Is better to explore X \cross Cost once a non-inf cost has been set
  bool sol_found = false;
  w_x_bk = aorrt_spec->w_x;
  w_c_bk = aorrt_spec->w_c;

  // This is equivalent to running RRT until the first solution is found
  aorrt_spec->w_x = 1.0;
  aorrt_spec->w_c = 0.0;
