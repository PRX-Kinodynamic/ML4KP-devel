...
void sst_t::_link_and_setup_spec(planner_specification_t* spec)
{
  ...
  witnesses = new graph_nearest_neighbors_t<space_point_t>(distance_function);
...
void sst_t::_resolve_query(condition_check_t* condition)
{
  ...
  do
  {
    ...
    std::transform(prox_nodes.begin(), prox_nodes.end(), std::back_inserter(nodes),
                   [](proximity_node_t<space_point_t>* prox_node) { return static_cast<sst_node_t*>(prox_node); });

    auto closest_node = *std::min_element(
        nodes.begin(), nodes.end(), [](sst_node_t* a, sst_node_t* b) { return a->cost_to_come < b->cost_to_come; });
    // expand
    ...
    if ((goal_vertex == start_vertex || closest_node->cost_to_come + edge_cost < current_solution))
    {
      ...
      if ((!closest_witness->set ||
           get_vertex(closest_witness->rep_index)->cost_to_come > closest_node->cost_to_come + edge_cost) &&
          valid_check(traj))
      {
        // add node
...
node_index_t sst_t::find_closest_witness(space_point_t s)
{
  ...
  if (witnesses->get_nr_nodes() == 0 ||
      distance_function(witness_tree.get_vertex_as<witness_node_t>(witness_index)->point, s) > sst_spec->delta_drain)
  {
    // create a new sample
