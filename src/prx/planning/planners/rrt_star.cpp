...
namespace prx
{
	rrt_star_t::rrt_star_t(const std::string& new_name) : rrt_t(new_name)
	{
	}
	...
	void rrt_star_t::_link_and_setup_spec(planner_specification_t* spec)
	{
		...
		rrt_star_spec = dynamic_cast<rrt_star_specification_t*>(spec);
		prx_assert(rrt_star_spec!=nullptr,"RRT received an incorrect specification.");
		steer = rrt_star_spec -> steer;

		steering_parameter = rrt_star_spec -> steering_parameter;
		radius = rrt_star_spec -> radius;
	...
	// void rrt_star_t::update_goal(node_index_t node_index)
	// {
	// 	auto new_tree_node = tree.get_vertex_as<rrt_node_t>(node_index);
	...
	// void rrt_star_t::bnb(node_index_t v, double cost_bound, bool delete_flag)
	// {
