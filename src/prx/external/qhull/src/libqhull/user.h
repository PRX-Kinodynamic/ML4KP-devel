...
/* Derived from Qt's corelib/global/qglobal.h */
    ...
    (defined(WIN64) || defined(_WIN64) || defined(__WIN64__) || defined(WIN32) || defined(_WIN32) ||                   \
...
/*-<a                             href="qh-user.htm#TOC"
  ...
  countT
    ...
    QH11026 FIX: countT may be defined as a 'unsigned int', but several code issues need to be solved first.  See countT
  in Changes.txt
*/
...
*/
#define qh_DISToutside ((qh_USEfindbestnew ? 2 : 1) * fmax_((qh MERGING ? 2 : 1) * qh MINoutside, qh max_outside))
...
/*-<a                             href="qh-user.htm#TOC"
  ...
  notes:
    As each point is added to the simplex, the max determinate is should approximate the previous determinate *
  qh.MAXwidth If maxdet is significantly less, the simplex may not be full-dimensional. If so, all points are searched,
  stopping at 10 times qh_RATIOmaxsimplex
*/
...
#define qh_SEARCHdist                                                                                                  \
  ((qh_USEfindbestnew ? 2 : 1) * (qh max_outside + 2 * qh DISTround + fmax_(qh MINvisible, qh MAXcoplanar)));
...
>--------------------------------</a><a name="WIDEpinched">-</a>
  ...
  qh_WIDEpinched
    Merge ratio for distance between pinched vertices compared to current facet width for qh_getpinchedmerges and
qh_next_vertexmerge Reports warning and merges duplicate ridges instead Enable these attempts with option Q14
merge-pinched-vertices
  ...
  notes:
    ...
    Found up to 45x ratio for qh_pointdist -- for ((i=1; i<20; i++)); do rbox 175 C1,6e-13 t | qhull d T4 2>&1 | tee x.1
| grep  -E 'QH|non-simplicial|Statis|pinched'; done Actual distance to facets is a third to a tenth of the qh_pointdist
(T1)
*/
