...
int main(int argc, char* argv[]) {
    ...
    // Add your existing code for initialization and setup here...
    ...
    // Other existing visualization code...
    // Visualization();
    prx::three_js_group_t* vis_group = new prx::three_js_group_t({ plant }, { obstacle_list });
    std::string body_name = params["/plant/name"].as<>() + "/" + params["/plant/vis_body"].as<>();


    // Add code to visualize the learned trajectory
    for (const auto& point : learned_trajectory) 
    {
        // Assuming the first three values in the point are x, y, z coordinates for the peg and the next four values are the quaternion: qw, qx, qy, qz
        std::vector<double> position(point.begin(), point.begin() + 3);
        std::vector<double> orientation(point.begin() + 3, point.begin() + 7);
        vis_group->add_vis_infos(prx::info_geometry_t::SPHERE, { position }, "0xff0000", 0.02); // red spheres
    }

    // Set the rrt_star_query.solution_traj to the learned trajectory
    //rrt_star_query.solution_traj = learned_trajectory;    
    // Assuming that prx::trajectory_t is a container of prx::space_point_t
    // and prx::space_point_t can be constructed from a std::vector<double>

    for (const auto& point : learned_trajectory) 
    {
        // Create a space_point_t from the point
        prx::space_point_t space_point = ss->make_point(); // Assuming ss is the state space

        // Now, assuming that the space_point can be directly assigned from a std::vector<double>
        // Copy the values from point to space_point
        std::copy(point.begin(), point.end(), space_point->memory);

        // Add the space point to the trajectory
        rrt_star_query.solution_traj.copy_onto_back(space_point);
    }

    // Note: The exact method to create a space_point_t and add it to the trajectory
    // will depend on the prx::space_t and prx::trajectory_t implementations.
    ...
    vis_group->output_html("peg_in_hole_learned_traj.html");
