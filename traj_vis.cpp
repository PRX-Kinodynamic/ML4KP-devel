...
prx::trajectory_t convertToTrajectory(const prx::space_t* space, const std::vector<std::vector<double>>& vec) {
    return prx::trajectory_t(space, vec); // Adjusted to use space and vec
}
...
int main(int argc, char* argv[])
{
    ...
    // Read and process the learned trajectory
    std::vector<std::vector<double>> learned_trajectory = read_trajectory("path_to_trajectory_file.txt");

    // Convert learned_trajectory to prx::trajectory_t
    prx::space_t* ss; // Make sure this is initialized correctly in your existing code
    prx::trajectory_t converted_trajectory = convertToTrajectory(ss, learned_trajectory);
